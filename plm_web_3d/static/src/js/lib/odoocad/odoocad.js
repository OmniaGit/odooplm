...
class OdooCAD {
    ...
    get_li_structure(object, nested = false) {
        ...
        for (let i = 0; i < object.children.length; i++) {
            ...
            if (
                child.name === "" &&
                !["Group", "Object3D", "Mesh"].includes(child.type)
            ) {
                continue;
            }
            ...
            var span_lable =
                "<span class='document_tree_span' webgl_ref_name='" +
                internal_obj_name +
                "'>" +
                obj_name +
                "</span>";
            ...
            if (children_found || child.name !== "") {
                ...
                const has_visible_children =
                    inner_html !== '<ul class="nested"></ul>' &&
                    inner_html !== '<ul id="myUL"></ul>';

                // Only push to HTML if it's not a geometry body
                if (!(child.name || "").toLowerCase().startsWith("body")) {
                    if (has_visible_children) {
                        out_lis +=
                            "<li class='document_tree_line' webgl_ref_name='" +
                            internal_obj_name +
                            "'><i class='tree_item_visibility fa fa-eye' aria-hidden='true'></i><span class='caret'>" +
                            span_lable +
                            "</span>" +
                            inner_html +
                            "</li>";
                        ...
                        out_lis +=
                            "<li class='document_tree_line' webgl_ref_name='" +
                            internal_obj_name +
                            "'><i class='tree_item_visibility fa fa-eye' aria-hidden='true'></i> " +
                            span_lable +
                            "</li>";
                    }
                }
    ...
    create_tree_structure(out_html_structure) {
        ...
        var html_out =
            "<div class='tree_structure' style='overflow-y: scroll;min-height: 1px;max-height: 400px;'>";
        ...
        for (let i = 0; i < hoverTargets.length; i++) {
            ...
            hoverTargets[i].addEventListener("click", function (event) {
                if (event.target.tagName != "I") {
                    ...
                    const product_tag = document.getElementById("linked_component_id");

                    if (product_tag && product_tag.dataset.id) {
                        const product_id = product_tag.dataset.id;
        ...
        // VISIBILITY TOGGLE
        // =========================
        var tree_item_visibility = document.getElementsByClassName(
            "tree_item_visibility"
        );
        ...
        for (let i = 0; i < tree_item_visibility.length; i++) {
            tree_item_visibility[i].addEventListener("click", function (event) {
                ...
                function objectsVisibility(items, visible, currentAttrValue) {
                    items.forEach((name) => {
                        ...
                        var groupDiv = document.querySelector(
                            `.document_tree_line[webgl_ref_name="${name}"]`
                        );
                ...
                const currentAttrValue = this.parentElement.getAttribute(
                    "webgl_ref_name"
                );
                const labelSpan = this.parentElement.querySelector(
                    ".document_tree_span"
                );
                if (!labelSpan) return;

                const labelContent = labelSpan.textContent;

                const matchingSpans = Array.from(
                    document.querySelectorAll(".document_tree_span")
                ).filter((span) => span.textContent === labelContent);

                const webglRefNames = matchingSpans
    ...
    create_relation_structure(object) {
        ...
        for (let i = 0; i < object.children.length; i++) {
            if (grp_types.includes(object.children[i].type)) {
                const [inner_html, _children] = self.get_li_structure(
                    object.children[i]
                );
                html_out += inner_html;
            }
    ...
    addDxfTextLabels(textEntities, origin) {
        ...
        const FONT_URL =
            "/plm_web_3d/static/src/js/lib/three.js/examples/fonts/helvetiker_regular.typeface.json";
        ...
        new FontLoader().load(FONT_URL, function (font) {
            for (const entry of textEntities) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    mesh.position.set(
                        entry.x - ox + dx - anchorX,
                        entry.y - oy - dy - anchorY,
                        0
                    );
