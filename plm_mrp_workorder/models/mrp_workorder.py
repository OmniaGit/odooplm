...
class MrpProductionWorkcenterLine(models.Model):
    ...
    quality_point_ids = fields.Many2many(
        "quality.point", compute="_compute_quality_point_ids", store=True
    )
    quality_point_count = fields.Integer(
        "Steps", compute="_compute_quality_point_count"
    )

    check_ids = fields.One2many("quality.check", "workorder_id")
    finished_product_check_ids = fields.Many2many(
        "quality.check", compute="_compute_finished_product_check_ids"
    )
    done_check_ids = fields.Many2many(
        "quality.check", compute="_compute_done_check_ids"
    )
    ...
    # QC-related fields
    allow_producing_quantity_change = fields.Boolean(
        "Allow Changes to Producing Quantity", default=True
    )

    is_last_lot = fields.Boolean("Is Last lot", compute="_compute_is_last_lot")
    is_first_started_wo = fields.Boolean(
        "Is The first Work Order", compute="_compute_is_last_unfinished_wo"
    )
    is_last_unfinished_wo = fields.Boolean(
        "Is Last Work Order To Process",
        compute="_compute_is_last_unfinished_wo",
        store=False,
    )
    lot_id = fields.Many2one(related="current_quality_check_id.lot_id", readonly=False)
    move_id = fields.Many2one(
        related="current_quality_check_id.move_id", readonly=False
    )
    move_line_id = fields.Many2one(
        related="current_quality_check_id.move_line_id", readonly=False
    )
    move_line_ids = fields.One2many(related="move_id.move_line_ids")
    quality_state = fields.Selection(
        related="current_quality_check_id.quality_state",
    ...
    user_id = fields.Many2one(
        related="current_quality_check_id.user_id", readonly=False
    )
    ...
    has_operation_note = fields.Boolean(
        "Has Description", compute="_compute_has_operation_note"
    )
    ...
    employee_ids = fields.Many2many(
        "hr.employee", string="Working employees", copy=False
    )
    ...
    # employees connected
    connected_employee_ids = fields.Many2many(
        "hr.employee", search="search_is_assigned_to_connected", store=False
    )
    ...
    def _compute_quality_point_ids(self):
        for workorder in self:
            ...
            quality_points = quality_points.filtered(
                lambda qp: (
                    not qp.product_ids
                    or workorder.production_id.product_id in qp.product_ids
                )
                and (qp.company_id == workorder.company_id)
            )
    ...
    def _compute_is_last_lot(self):
        for wo in self:
            ...
            wo.is_last_lot = (
                float_compare(
                    wo.qty_producing, wo.qty_remaining, precision_rounding=precision
                )
                >= 0
            )
    ...
    def _compute_is_last_unfinished_wo(self):
        for wo in self:
            wo.is_first_started_wo = all(
                wo.state != "done" for wo in (wo.production_id.workorder_ids - wo)
            )
    ...
    def _compute_done_check_ids(self):
        for wo in self:
            wo.done_check_ids = wo.check_ids.filtered(
    ...
    def action_back(self):
        ...
        if self.env.context.get("from_manufacturing_order"):
            ...
            action = self.env["ir.actions.actions"]._for_xml_id(
                "plm_mrp_workorder.mrp_workorder_action_tablet"
            )
            ...
            action["target"] = "main"
            action["context"] = dict(
                literal_eval(action["context"]), no_breadcrumbs=True
            )
        else:
    ...
    def _create_subsequent_checks(self):
        ...
        if (
            next_check.component_id != self.current_quality_check_id.product_id
            or next_check.point_id != self.current_quality_check_id.point_id
        ):
    ...
    def _change_quality_check(self, position):
        ...
        checks_to_consider = self.check_ids.filtered(
            lambda c: c.quality_state == "none"
        )
        if position == "first":
            check = checks_to_consider.filtered(
                lambda check: not check.previous_check_id
            )
        elif position == "next":
        ...
        self.write(
            {
                "allow_producing_quantity_change": not check.previous_check_id.filtered(
    ...
    def action_menu(self):
        return {
            ...
            "views": [
                [
                    self.env.ref(
                        "plm_mrp_workorder.mrp_workorder_view_form_tablet_menu"
                    ).id,
                    "form",
                ]
            ],
    ...
    def button_start(self, raise_on_invalid_state=False, bypass=False):
        ...
        if not skip_employee_check:
            if not self.env.context.get("mrp_display"):
                ...
                if not self.env.user.employee_id:
                    raise UserError(
                        _(
                            "You need to link this user to an employee of this company to process the work order"
                        )
                    )
            else:
            ...
            if any(
                main_employee not in [emp.id for emp in wo.allowed_employees]
                and not wo.all_employees_allowed
                for wo in self
            ):
        ...
        for wo in self:
            ...
            if main_employee:
                if (
                    len(wo.allowed_employees) == 0
                    or main_employee in [emp.id for emp in wo.allowed_employees]
    ...
    def verify_quality_checks(self):
        for check in self.check_ids:
            ...
            if check.test_type in [
                ...
                raise UserError(
                    _(
                        "You need to complete Quality Checks using the Shop Floor before marking Work Order as Done."
                    )
                )

    def action_propose_change(self, change_type, check_id):
        change_type_to_title = {
            "update_step": "Update Instructions",
            "remove_step": "Remove Step",
            "set_picture": "Change Picture",
        }
        return {
            "type": "ir.actions.act_window",
            ...
            "views": [
                [
                    self.env.ref("plm_mrp_workorder.view_propose_change_wizard").id,
                    "form",
                ]
            ],
    ...
    def action_add_step(self):
        ...
        return {
            ...
            "views": [
                [
                    self.env.ref("plm_mrp_workorder.add_quality_check_from_tablet").id,
                    "form",
                ]
            ],
            "name": _("Add a Step"),
            "target": "new",
            "context": {
                "default_test_type_id": self.env.ref(
    ...
    def action_open_mes(self):
        action = self.env["ir.actions.actions"]._for_xml_id(
            "plm_mrp_workorder.action_mrp_display"
        )
        action["context"] = {
    ...
    def _create_checks(self):
        for wo in self:
            ...
            move_finished_ids = wo.move_finished_ids.filtered(
                lambda m: m.state not in ("done", "cancel")
                and m.product_id != wo.production_id.product_id
            )
                    ...
                    if point.test_type == "register_byproducts":
                        ...
                        if not moves:
                            moves = production.move_finished_ids.filtered(
                                ...
                                and m.product_id == point.component_id
                            )
    ...
    def pre_record_production(self):
        ...
        self._check_company()
        if any(
            x.quality_state == "none"
            for x in self.check_ids
            if x.test_type != "instructions"
        ):
            raise UserError(_("You still need to do the quality checks!"))
        if (
            float_compare(
                self.qty_producing, 0, precision_rounding=self.product_uom_id.rounding
            )
            <= 0
        ):
            raise UserError(
                _(
                    "Please set the quantity you are currently producing. It should be different from zero."
                )
            )
    ...
    def _defaults_from_move(self, move):
        ...
        move_line_id = move.move_line_ids.filtered(
            lambda sml: sml._without_quality_checks()
        )[:1]
        if move_line_id:
            vals.update(
                {
    ...
    def open_tablet_view(self):
        ...
        if self._should_start():
            self.button_start()
        action = self.env["ir.actions.actions"]._for_xml_id(
            "plm_mrp_workorder.tablet_client_action"
        )
    ...
    def do_finish(self):
        ...
        if self.state != "done":
            ...
            if len(loss_id) < 1:
                raise UserError(
                    _(
                        "You need to define at least one productivity loss in the category 'Productive'. Create one from the Manufacturing app, menu: Configuration / Productivity Losses."
                    )
                )
    ...
    def get_workorder_data(self):
        ...
        data = {
            ...
            "operation": self.operation_id.read(
                self.operation_id._get_fields_for_tablet()
            )[0]
            if self.operation_id
            else {},
            ...
            "views": {
                "workorder": self.env.ref(
                    "plm_mrp_workorder.mrp_workorder_view_form_tablet"
                ).id,
                "check": self.env.ref(
                    "plm_mrp_workorder.quality_check_view_form_tablet"
                ).id,
            },
        }
        ...
        data.update(
            {
                ...
                "employee_list": self.env["hr.employee"].search_read(
                    employee_domain, fields_to_read, load=False
                ),
            }
        )
        return data
    ...
    def get_summary_data(self):
        ...
        last30op = self.env["mrp.workorder"].search_read(
            [
                ...
                (
                    "date_finished",
                    ">",
                    fields.datetime.today() - relativedelta(days=30),
                ),
            ],
            ["duration", "qty_produced"],
        )
        last30op = sorted(
            [item["duration"] / item["qty_produced"] for item in last30op]
        )
        ...
        if last30op:
            show_rainbow = (
                show_rainbow
                and float_compare(
                    (self.duration / self.qty_producing),
                    last30op[0],
                    precision_digits=2,
                )
                <= 0
        ...
        if self.check_ids:
            passed_checks = len(
                [check for check in self.check_ids if check.quality_state == "pass"]
            )
    ...
    def _action_confirm(self):
        ...
        self.filtered(
            lambda wo: wo.state != "cancel" and not wo.check_ids
        )._create_checks()
        return res
    ...
    def _web_gantt_reschedule_is_record_candidate(
        ...
        return self.state not in [
            ...
            "done",
            "cancel",
        ] and super()._web_gantt_reschedule_is_record_candidate(
            start_date_field_name, stop_date_field_name
        )

    def _web_gantt_reschedule_is_relation_candidate(
        self, master, slave, start_date_field_name, stop_date_field_name
    ):
        return self != slave and super()._web_gantt_reschedule_is_relation_candidate(
    ...
    def _web_gantt_reschedule_compute_dates(
        ...
        from_date, to_date = self.workcenter_id._get_first_available_slot(
            ...
            leaves_to_ignore=self.leave_id,
        )
    ...
    def _web_gantt_reschedule_write_new_dates(
        ...
        )._web_gantt_reschedule_write_new_dates(
    ...
    def _web_gantt_progress_bar_workcenter_id(self, res_ids, start, stop):
        ...
        (
            workcenters_work_intervals,
            dummy,
        ) = workcenters.resource_id._get_valid_work_intervals(start, stop)
        for workorder in workorders:
            max_start = max(start, utc.localize(workorder.date_start))
            min_finished = min(stop, utc.localize(workorder.date_finished))
            interval = Intervals(
                [(max_start, min_finished, self.env["resource.calendar.attendance"])]
            )
            work_intervals = (
                interval
                & workcenters_work_intervals[workorder.workcenter_id.resource_id.id]
            )
            planned_hours[workorder.workcenter_id] += sum_intervals(work_intervals)
        work_hours = {
            id: sum_intervals(work_intervals)
            for id, work_intervals in workcenters_work_intervals.items()
        }
        return {
            workcenter.id: {
    ...
    def get_gantt_data(
        ...
        gantt_data = super(
            MrpProductionWorkcenterLine, self.with_context(prefix_product=True)
        ).get_gantt_data(
            domain,
            groupby,
            read_specification,
        ...
        if "workcenter_id" not in gantt_data["unavailabilities"]:
            ...
            if groupby and "workcenter_id" in groupby:
                for group in gantt_data["groups"]:
                    res_id = (
                        group["workcenter_id"][0] if group["workcenter_id"] else False
                    )
                    workcenter_ids.add(res_id)
            else:
                for record in gantt_data["records"]:
                    res_id = (
                        record["workcenter_id"]["id"]
                        if record.get("workcenter_id")
                        else False
                    )
                    workcenter_ids.add(res_id)
            start, stop = fields.Datetime.from_string(
                start_date
            ), fields.Datetime.from_string(stop_date)
            gantt_data["unavailabilities"][
                "workcenter_id"
            ] = self._gantt_unavailability(
                "workcenter_id", workcenter_ids, start, stop, scale
            )
        return gantt_data
    ...
    def _gantt_progress_bar(self, field, res_ids, start, stop):
        ...
        if field == "workcenter_id":
            return dict(
                ...
                warning=_(
                    "This workcenter isn't expected to have open workorders during this period. Work hours :"
                ),
            )
    ...
    def _should_be_pending(self):
        return (
            self.is_user_working
            and self.working_state != "blocked"
            and len(self.employee_ids.ids) == 0
        )

    def _should_start(self):
        if self.working_state != "blocked" and self.state in (
            "ready",
            "waiting",
            "progress",
            "pending",
    ...
    def _compute_duration(self):
        ...
        return super(
            MrpProductionWorkcenterLine,
            self.env["mrp.workorder"].browse(wo_ids_without_employees),
        )._compute_duration()
    ...
    def _compute_employee_id(self):
        ...
        self.employee_name = (
            self.env["hr.employee"].browse(main_employee_connected).name
        )
    ...
    def start_employee(self, employee_id):
        self.ensure_one()
        if employee_id in self.employee_ids.ids and any(
            not t.date_end for t in self.time_ids if t.employee_id.id == employee_id
        ):
            return
    ...
    def get_duration(self):
        ...
        for dummy, times in loss_type_times.items():
            duration += self._intervals_duration(
                [(t.date_start, t.date_end or now, t) for t in times]
            )
        return duration
    ...
    def get_working_duration(self):
        self.ensure_one()
        now = fields.Datetime.now()
        return self._intervals_duration(
            [(t.date_start, now, t) for t in self.time_ids if not t.date_end]
        )
    ...
    def get_productive_duration(self):
        ...
        duration = self._intervals_duration(
            [(t.date_start, t.date_end or now, t) for t in productive_times]
        )
        return duration

    def _cal_cost(self, date=False):
        if date:
            return super()._cal_cost(date) + sum(
    ...
    def action_mark_as_done(self):
        ...
        for wo in self:
            if not main_employee_connected:
                raise UserError(
                    _("You must be logged in to process some of these work orders.")
                )
            if len(wo.allowed_employees) != 0 and main_employee_connected not in [
                wo.id for wo in wo.allowed_employees
            ]:
                raise UserError(
                    _("You are not allow to work on some of these work orders.")
                )
        ...
        for wo in self:
            if not wo.time_ids:
                ...
                date_start = datetime.fromtimestamp(
                    now.timestamp() - ((wo.duration_expected * 60) // 1)
                )
                date_end = now
    ...
    def _compute_expected_operation_cost(self, without_employee_cost=False):
        ...
        expected_labour_cost = (
            (self.duration_expected / 60)
            * self.workcenter_id.employee_costs_hour
            * (self.operation_id.employee_ratio or 1)
        )
    ...
    def action_mrp_workorder_dependencies(self, action_name):
        ...
        ref = (
            "plm_mrp_workorder.workcenter_line_gantt_production_dependencies"
            if action_name == "production"
            else "plm_mrp_workorder.mrp_workorder_view_gantt_dependencies"
        )
