...
class TestType(models.Model):
    ...
    allow_registration = fields.Boolean(
        search="_get_domain_from_allow_registration", store=False, default=False
    )
...
class MrpRouting(models.Model):
    ...
    quality_point_count = fields.Integer(
        "Instructions", compute="_compute_quality_point_count"
    )
    ...
    def _compute_operation_cost(self):
        expected_employee_cost = (
            (self.time_cycle / 60)
            * self.workcenter_id.employee_costs_hour
            * (self.employee_ratio or 1)
        )
    ...
    def copy(self, default=None):
        ...
        if default and "bom_id" in default:
            for new_workcenter in new_workcenters:
                new_workcenter.quality_point_ids._change_product_ids_for_bom(
                    new_workcenter.bom_id
                )
    ...
    def action_mrp_workorder_show_steps(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "plm_mrp_workorder.action_mrp_workorder_show_steps"
        )
        ctx = {
...
class QualityPoint(models.Model):
    ...
    def _default_product_ids(self):
        ...
        if operation_id:
            ...
            return (
                bom.product_id.ids
                if bom.product_id
                else bom.product_tmpl_id.product_variant_id.ids
            )

    is_workorder_step = fields.Boolean(compute="_compute_is_workorder_step")
    operation_id = fields.Many2one("mrp.routing.workcenter", "Step", check_company=True)
    bom_id = fields.Many2one(related="operation_id.bom_id")
    bom_active = fields.Boolean(
        "Related Bill of Material Active", related="bom_id.active"
    )
    ...
    bom_product_ids = fields.One2many(
        "product.product", compute="_compute_bom_product_ids"
    )
    ...
    source_document = fields.Selection(
        selection=[
            ("operation", "Specific Page of Operation Worksheet"),
            ("step", "Custom"),
        ],
    ...
    def _compute_bom_product_ids(self):
        ...
        for point in points_for_workorder_step:
            bom_product_ids = (
                point.bom_id.product_id
                or point.bom_id.product_tmpl_id.product_variant_ids
            )
            point.bom_product_ids = bom_product_ids.filtered(
                lambda p: not p.company_id or p.company_id == point.company_id._origin
            )
    ...
    def _compute_component_ids(self):
        ...
        for point in self:
            if point.test_type == "register_byproducts":
                ...
                point.component_ids = self.env["product.product"].browse(
                    component_product_ids
                )
    ...
    def _compute_is_workorder_step(self):
        for quality_point in self:
            quality_point.is_workorder_step = quality_point.picking_type_ids and all(
                pt.code == "mrp_operation" for pt in quality_point.picking_type_ids
            )
    ...
    def _onchange_test_type_id(self):
        if self.test_type_id.technical_name not in (
            "register_byproducts",
            "register_consumed_materials",
        ):
...
class QualityCheck(models.Model):
    ...
    move_id = fields.Many2one("stock.move", "Stock Move", check_company=True)
    ...
    component_uom_id = fields.Many2one(
        "uom.uom", related="move_id.product_uom", readonly=True
    )
    ...
    additional = fields.Boolean(
        "Register additional product", compute="_compute_additional"
    )
    component_tracking = fields.Selection(
        related="component_id.tracking", string="Is Component Tracked"
    )

    # Workorder specific fields
    component_remaining_qty = fields.Float(
        "Remaining Quantity for Component",
        compute="_compute_component_data",
        digits="Product Unit of Measure",
    )
    component_qty_to_do = fields.Float(
        compute="_compute_component_qty_to_do", digits="Product Unit of Measure"
    )
    ...
    def _compute_title(self):
        ...
        for check in self:
            if not check.point_id and check.component_id:
                check.title = '{} "{}"'.format(
                    check.test_type_id.display_name,
                    check.component_id.name or check.workorder_id.name,
                )
    ...
    def _get_check_result(self):
        if (
            self.test_type in ("register_consumed_materials", "register_byproducts")
            and self.lot_id
        ):
            return "{} - {}, {} {}".format(
            ...
            return "{}, {} {}".format(
                self.component_id.name, self.qty_done, self.component_uom_id.name
            )
        else:
            return ""
    ...
    def _compute_component_data(self):
        ...
        for check in self:
            if check.test_type in (
                ...
                if check.quality_state == "none":
                    completed_lines = check.workorder_id.move_line_ids.filtered(
                        lambda l: l.picked
                        and (check.component_id.tracking == "none" or l.lot_id)
                    )
                    if check.move_id.additional:
                        qty = check.workorder_id.qty_remaining
                    else:
                        qty = (
                            check.workorder_id.qty_producing
                            or check.workorder_id.qty_remaining
                        )
                    check.component_remaining_qty = self._prepare_component_quantity(
                        check.move_id, qty
                    ) - sum(completed_lines.mapped("quantity"))
    ...
    def _get_print_qty(self):
        if self.product_id.uom_id.category_id == self.env.ref(
    ...
    def _get_lot_label_action(self, report_type):
        ...
        res = self.env.ref(xml_id).report_action(
            [self.workorder_id.finished_lot_id.id] * qty
        )
    ...
    def add_check_in_chain(self, notify_bom=True):
        ...
        if notify_bom and self.workorder_id.production_id.bom_id:
            body = _(
                "BoM feedback (%(production)s - %(operation)s)",
                production=self.workorder_id.production_id.name,
                operation=self.workorder_id.operation_id.name,
            )
            body += Markup("<br/>%s") % _(
                "New Step suggested by %(user_name)s", user_name=self.env.user.name
            )
            if self.title:
            ...
            if self.worksheet_url:
                body += Markup("<br/><a href='%s'>%s</a>") % (
                    self.worksheet_url,
                    _("Google Doc"),
                )
            self.workorder_id.production_id.bom_id.message_post(
    ...
    def _create_extra_move_lines(self):
        ...
        location_dest_id = self.move_id.location_dest_id._get_putaway_strategy(
            self.move_id.product_id
        )
        quants = self.env["stock.quant"]._gather(
            self.product_id, self.move_id.location_id, lot_id=self.lot_id, strict=False
        )
        ...
        for quant in quants:
            ...
            quantity = self.component_id.uom_id._compute_quantity(
                quantity, move_uom, rounding_method="HALF-UP"
            )
            rounding = quant.product_uom_id.rounding
            if (
                float_compare(quant.quantity, 0, precision_rounding=rounding) <= 0
                or float_compare(quantity, 0, precision_rounding=move_uom.rounding) <= 0
            ):
                continue
            vals.update(
                {
            ...
            if (
                float_compare(
                    qty_done, 0, precision_rounding=self.product_id.uom_id.rounding
                )
                <= 0
            ):
                break
            ...
            def _find_quant(lot, location):
                quants = lot.quant_ids.filtered(
                    lambda quant: float_compare(
                        quant.quantity, 0, precision_rounding=rounding
                    )
                    > 0
                    and float_compare(
                        quant.quantity,
                    ...
                    and (
                        quant.location_id._child_of(self.move_id.location_id)
                        if self.move_id
                        else True
                    )
                )
            ...
            if self.move_line_id:
                ...
                if self.move_line_id.product_id.tracking != "none":
                    ...
                    if (
                        ...
                        and self.move_line_id.lot_id != self.lot_id
                    ):
                        quant = _find_quant(
                            lot=self.lot_id, location=self.move_line_id.location_id
                        )
                        if quant:
                ...
                if (
                    ...
                    if (
                        continue_production
                        and float_compare(
                            new_qty_reserved, 0, precision_rounding=rounding
                        )
                        >= 0
                    ):
                        self.move_line_id.copy(
                            default={"quantity": new_qty_reserved, "picked": new_picked}
                        )
            else:
                sml_vals = self._create_extra_move_lines()
                if self.test_type == "register_consumed_materials" and self.lot_id:
                    quant = _find_quant(
                        lot=self.lot_id, location=self.env["stock.location"]
                    )
                    if quant:
                        for val in sml_vals:
    ...
    def _update_component_quantity(self):
        if self.component_tracking == "serial":
            self._origin.qty_done = self.component_id.uom_id._compute_quantity(
                1, self.component_uom_id, rounding_method="HALF-UP"
            )
            return
        move = self.move_id
        ...
        new_qty = self._prepare_component_quantity(
            move, self.workorder_id.qty_producing
        )
