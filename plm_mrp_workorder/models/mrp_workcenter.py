...
class MrpWorkcenter(models.Model):
    ...
    employee_costs_hour = fields.Monetary(
        string="Employee Hourly Cost", currency_field="currency_id", default=0.0
    )

    def action_work_order(self):
        if self.env.user.has_group(
            "plm_mrp_workorder.group_mrp_wo_shop_floor"
        ) and not self.env.context.get("desktop_list_view", False):
...
class MrpWorkcenterProductivity(models.Model):
    ...
    employee_cost = fields.Monetary(
        "employee_cost", compute="_compute_employee_cost", default=0, store=True
    )
