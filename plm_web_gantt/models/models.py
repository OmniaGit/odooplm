...
class Base(models.AbstractModel):
    ...
    def get_gantt_data(
        ...
        for field in unavailability_fields:
            unavailabilities[field] = self._gantt_unavailability(
                field, list(res_ids_for_unavailabilities[field]), start, stop, scale
            )
        ...
        for field in progress_bar_fields:
            progress_bars[field] = self._gantt_progress_bar(
                field, list(res_ids_for_progress_bars[field]), start, stop
            )
    ...
    def web_gantt_reschedule(
        ...
        if not self._web_gantt_reschedule_is_relation_candidate(
            ...
            return {
                "type": "warning",
                "message": _(
                    "You cannot reschedule %(main_record)s towards %(other_record)s.",
                    main_record=master_record.name,
                    other_record=slave_record.name,
                ),
            }
        ...
        if not trigger_record._web_gantt_reschedule_is_record_candidate(
        ...
        ) = trigger_record._web_gantt_action_reschedule_candidates(
            dependency_field_name,
            dependency_inverted_field_name,
            start_date_field_name,
            stop_date_field_name,
    ...
    def _web_gantt_get_candidates(
        ...
        stop_date_field_name,
        related_record,
        move_forward_without_conflicts,
    ):
        result = {
            "warnings": [],
            "errors": [],
        ...
        if move_forward_without_conflicts:
            ...
            candidates_to_exclude = {self.id} | set(
                related_record[dependency_inverted_field_name].ids
            )
        ...
        if move_forward_without_conflicts:
            candidates_to_exclude = {related_record.id} | set(
                self[dependency_field_name].ids
            )
        else:
        ...
        if move_forward_without_conflicts:
            all_pills_ids, pills_to_check_from_ids = self_children_ids, set(
                related_record_ancestors_ids
            )
        else:
            related_record_ancestors_ids.reverse()
            all_pills_ids, pills_to_check_from_ids = (
                related_record_ancestors_ids,
                self_children_ids,
            )
        ...
        for pill_id in all_pills_ids:
            if pill_id in pills_to_check_from_ids:
                (
                    pills_to_plan_before
                    if move_forward_without_conflicts
                    else pills_to_plan_after
                ).append(pill_id)
            else:
                (
                    pills_to_plan_after
                    if move_forward_without_conflicts
                    else pills_to_plan_before
                ).append(pill_id)
    ...
    def _web_gantt_action_reschedule_candidates(
        ...
        if (
            search_forward
            and self[stop_date_field_name] <= related_record[start_date_field_name]
            and related_record in self[dependency_inverted_field_name]
        ):
            (
                log_messages,
                pills_to_plan_before_related_record,
                pills_to_plan_after_related_record,
                all_candidates_ids,
            ) = self._web_gantt_get_candidates(
                dependency_field_name,
                dependency_inverted_field_name,
                start_date_field_name,
                stop_date_field_name,
            ...
            if log_messages.get("errors") or not pills_to_plan_after_related_record:
                return (
                    log_messages,
                    {} if log_messages.get("errors") else old_vals_per_pill_id,
                )

            # plan related_record_ancestors forward from related_record
            (
                new_log_messages,
                second_old_vals_per_pill_id,
            ) = self._web_gantt_move_candidates(
                start_date_field_name,
                stop_date_field_name,
                dependency_field_name,
                dependency_inverted_field_name,
            ...
            log_messages.setdefault("errors", []).extend(
                new_log_messages.get("errors", [])
            )
            log_messages.setdefault("warnings", []).extend(
                new_log_messages.get("warnings", [])
            )

            return log_messages, old_vals_per_pill_id | second_old_vals_per_pill_id
        # moving backward without conflicts
        elif (
            related_record[stop_date_field_name] <= self[start_date_field_name]
            and related_record in self[dependency_field_name]
        ):
            (
                log_messages,
                pills_to_plan_before_related_record,
                pills_to_plan_after_related_record,
                all_candidates_ids,
            ) = related_record._web_gantt_get_candidates(
                dependency_field_name,
                dependency_inverted_field_name,
                start_date_field_name,
                stop_date_field_name,
        ...
        # moving forward or backward with conflicts
            ...
            return self._web_gantt_move_candidates(
                ...
                candidates_ids,
                related_record[
                    stop_date_field_name if search_forward else start_date_field_name
                ],
            )
    ...
    def _web_gantt_is_candidate_in_conflict(
        ...
        return any(
            r[start_date_field_name]
            and r[stop_date_field_name]
    ...
    def _web_gantt_move_candidates(
        ...
        candidates_ids,
        date_candidate=None,
        all_candidates_ids=None,
        move_not_in_conflicts_candidates=False,
    ):
        ...
        for i, candidate in enumerate(candidates):
            if (
                not move_not_in_conflicts_candidates
            ...
            start_date, end_date = start_date.astimezone(
                timezone.utc
            ...
            if not candidate._web_gantt_reschedule_write_new_dates(
                start_date, end_date, start_date_field_name, stop_date_field_name
            ):
    ...
    def _web_gantt_check_cycle_existance_and_get_rescheduling_candidates(
        ...
        :param dependency_field_name: The field name of the relation between the master
         and slave records.
        :param dependency_inverted_field_name: The field name of the relation between
               the slave and the parent records.
        :param start_date_field_name: The start date field used in the gantt view.
        :param stop_date_field_name: The stop date field used in the gantt view.
        ...
        :return: bool, True if there is a cycle, else False.
            candidates_id will also contain the pills to plan in a valid topological
            order
        """
        ...
        for child in self[dependency_field_name]:
            ...
            if (
                ...
                and child.id not in candidates_to_exclude
        ...
        ancestors.pop()
        if (
            self._web_gantt_reschedule_is_record_candidate(
                start_date_field_name, stop_date_field_name
            )
            and self.id not in candidates_to_exclude
        ):
    ...
    def _web_gantt_reschedule_compute_dates(
        ...
        This method is meant to be overridden when we need to add constraints that have
        to be taken into account
        in the computing of the start_date and end_date.

        :param date_candidate: The optimal date, which does not take any constraint
         into account.
        :param start_date_field_name: The start date field used in the gantt view.
        :param stop_date_field_name: The stop date field used in the gantt view.
    ...
    def _web_gantt_reschedule_is_in_conflict(
        ...
        """Get whether the dependency relation between a master and a slave record is in
        conflict.
        This check is By-passed for slave records if moving records forwards and the for
        master records if moving records backwards
        (see _web_gantt_get_rescheduling_candidates and
        _web_gantt_reschedule_is_in_conflict_or_force). In order to add condition that
        would not be
    ...
    def _web_gantt_reschedule_is_in_conflict_or_force(
        ...
        conflict.
        This check is By-passed for slave records if moving records forwards and the for
        master records if moving records backwards. In order to add condition that would
        not be by-passed, rather consider _web_gantt_reschedule_is_relation_candidate.

        This def purpose is to be able to prevent the default behavior in some modules
        by overriding the def and forcing / preventing the rescheduling il all
        circumstances if needed. See _web_gantt_get_rescheduling_candidates.
    ...
    def _web_gantt_reschedule_is_record_candidate(
        ...
        """Get whether the record is a candidate for the rescheduling. This method is
        meant to be overridden when we need to add a constraint in order to prevent some
        records to be rescheduled. This method focuses on the record itself(if you need
        to have information on the relation (master and slave) rather override
    ...
    def _web_gantt_reschedule_is_relation_candidate(
        ...
        """Get whether the relation between master and slave is a candidate for the
        rescheduling. This method is meant to be overridden when we need to add a
        constraint in order to prevent some records to be rescheduled.
        This method focuses on the relation between records (if your logic is rather on
        one record, rather override
        _web_gantt_reschedule_is_record_candidate).

        :param master: The master record we need to evaluate whether it is a candidate
        for rescheduling or not.
        :param slave: The slave record.
        :param start_date_field_name: The start date field used in the gantt view.
        :param stop_date_field_name: The stop date field used in the gantt view.
        :return: True if record can be rescheduled, False if not.
    ...
    def _web_gantt_reschedule_write_new_dates(
        ...
        epsilon = 30 seconds was added because the first valid interval can be now and
        because of some seconds, it will become < now() at the comparaison moment
