...
class ProductProductExtension(models.Model):
    ...
    def _create_bom_from_ebom(
        ...
        if bom_brws_list:
            ...
            eng_bom_brws_list = bom_type.search(
                [("product_tmpl_id", "=", product_template_id), ("type", "=", "ebom")],
                order="engineering_revision DESC",
                limit=1,
            )
            if not eng_bom_brws_list:
                logging.info(
                    "No EBOM or NBOM found for template id: {}".format(
                        product_template_id
                    )
                )
                return []
            for e_bom_brws in eng_bom_brws_list:
                ...
                values = {
                    "name": obj_product_product_brw.name,
                    "product_tmpl_id": product_template_id,
                    "type": new_bom_type,
            ...
            for new_line in new_bom_brws.bom_line_ids:
                if new_line.product_id.id == product_id:
                    ...
                    if summarize:
                        out_msg = (
                            out_msg
                            + 'In BOM "%s" line "%s" has been summarized.'
                        ...
                        out_msg = (
                            out_msg
                            + 'In BOM "%s" line "%s" has been not summarized.'
                            % (template_name, product_name)
                        )
        ...
        for old_brws_line in old_n_bom.bom_line_ids:
            if not old_brws_line.ebom_source_id:
                qty, found_line_id = verify_summarize(
                    old_brws_line.product_id.id, old_brws_line.product_qty
                )
    ...
    def _create_normalBom(self, idd, processedIds):
        ...
        obj_boms = bom_type.search(
            [("product_tmpl_id", "=", product_template_id), ("type", "=", "normal")]
        )
        if not obj_boms:
            bom_brws_list = bom_type.search(
                [("product_tmpl_id", "=", product_template_id), ("type", "=", "ebom")]
