...
class PlmDocumentActionSyncronize(models.Model):
    ...
    def syncronize(self, document_ids=[]):
        ...
        for plm_document_action_syncronize_id in self.search(
            ...
            if not plm_document_action_syncronize_id.done:
                ...
                if ir_attachment_id.id not in ir_attachment_processed:
                    if plm_document_action_syncronize_id.plm_remote_server_id.push_document_to_remote(
                        ir_attachment_id
                    ):
            ...
            self.env.cr.commit()
        ir_attachment_processed = []
        domain_pull = [("done", "=", False), ("action", "=", "pull")]
        if document_ids:
        ...
        for plm_document_action_syncronize_id in self.search(
            domain_pull, order="id desc"
        ):
            ir_attachment_id = plm_document_action_syncronize_id.ir_attachment_id
            if ir_attachment_id.id not in ir_attachment_processed:
                if plm_document_action_syncronize_id.plm_remote_server_id.pull_document_to_odoo(
                    plm_document_action_syncronize_id.ir_attachment_id
                ):
                    plm_document_action_syncronize_id.done = True
                    ir_attachment_processed.append(ir_attachment_id.id)
            else:
