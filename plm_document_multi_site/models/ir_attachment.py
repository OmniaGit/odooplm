...
class IrAttachment(models.Model):
    ...
    plm_document_action_syncronize_ids = fields.One2many(
        "plm.document.action.syncronize", inverse_name="ir_attachment_id"
    )

    count_plm_document_action_syncronize = fields.Integer(
        compute="_count_plm_document_action_syncronize_ids_count",
        string="S.N",
    ...
    def _count_plm_document_action_syncronize_ids_count(self):
        ...
        for ir_attachment_id in self:
            ir_attachment_id.count_plm_document_action_syncronize = 0 or self.env[
                "plm.document.action.syncronize"
            ].search_count(
                [("ir_attachment_id", "=", ir_attachment_id.id), ("done", "=", False)]
            )
    ...
    def thereIsPendingSyncronizations(self, plm_remote_server_id):
        if plm_remote_server_id:
            return (
                self.env["plm.document.action.syncronize"].search_count(
                    [
    ...
    def create_server_syncronize(self, args):
        ...
        for plm_remote_server_id in self.env["plm.remote.server"].search([]):
            ...
            if action:
                ...
                raise UserError(
                    "unable to find a correct action to perform the syncronization %s"
                    % document_id
                )
        return True
    ...
    def _isDownloadableFromServer(self, server_name):
        ...
        """
        plm_remote_server_id = self.env["plm.remote.server"].search(
            [("name", "=", server_name)]
        )
        pending_syncronization = self.thereIsPendingSyncronizations(
            plm_remote_server_id
        )
        if not pending_syncronization:
            if not plm_remote_server_id.document_is_there(self.id):
                self.create_server_syncronize((self.id, "odoo"))
                return False, "Document %r is no syncronized on server %s " % (
                    self.name,
                    server_name,
                )
            return True, ""
    ...
    def canCheckOut(self, showError=False):
        ret = super().canCheckOut(showError=showError)
        for (
            plm_document_action_syncronize_id
        ) in self.plm_document_action_syncronize_ids:
