...
class MailActivity(models.Model):
    ...
    children_ids = fields.One2many(
        "mail.activity.children.rel", "mail_parent_activity_id", "ECR Activities"
    )
    ...
    eco_child_ids = fields.One2many(
        "mail.activity", "mail_parent_eco_activity_id", "ECO Activities"
    )
    mail_parent_eco_activity_id = fields.Many2one(
        "mail.activity", "ECO Parent Activity"
    )
    ...
    def activity_format(self):
        out = []
        super_res = super().activity_format()
        if (
            self.activity_type_id
            == self.env.ref("activity_validation.mail_activity_change_request").id
        ):
