...
class BookCollector(object):
    ...
    def getNextPageNumber(self, mediaBox, docState):
        ...
        if isinstance(self.customTest, tuple):
            ...
            if page:
                msg = "Page: " + unicode(self.pageCount) + unicode(message)
                cha = len(msg)
                c.drawRightString(float(x1) - cha, self.bottomHeight, " Page: " + unicode(self.pageCount))
                c.drawString(float(x) + 20, self.bottomHeight, unicode(message))
            else:
                cha = len(str(message))
                c.drawString(float(x) + 20, self.bottomHeight, unicode(message))
        else:
