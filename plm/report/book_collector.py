...
def packDocumentsNew(docRepository, documents, bookCollector):
    """
...
def getPagePaperFormat(product_id, byteIoStream):
    #
    reader = PdfFileReader(byteIoStream, strict=False)
...
def extract_page_name(file_orig, name):
...
def extract_pdf_from_name(reader, name):
    page_number = -1
