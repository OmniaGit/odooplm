...
class ReportProdStructure(models.AbstractModel):
        ...
        for product_id in product_ids:
            for attachment_id in product_id.linkeddocuments:
                if attachment_id.document_type.upper() == "3D":
                    ...
                    vals = self.singleItem(
                        attachment_id, 0, True, items_to_show, attachment_id
                    )
                    docs.append(vals)
        return {
            "doc_ids": docids,
    ...
    def get_doc_prod(self, attachment, level=False, items_to_show={}, root_doc=False):
        ...
        return self.env["ir.ui.view"]._render_template(
            "plm.report_prod_bom_line", {"data": data}
        )
    ...
    def checkVisibleItems(self, parent_doc, parents=[]):
        ...
        if not parent_doc.linkedcomponents:
            ...
            out.extend(parents)
        children = parent_doc.getRelatedOneLevelLinks(
            parent_doc.id, ["RfTree", "HiTree"]
        )
    ...
    def singleItem(
        self, attachment, level=0, recursion=True, items_to_show={}, root_doc=False
    ):
        children_list = []
        visible = False
        ...
        if recursion and attachment.document_type.upper() == "3D":
            children = attachment.getRelatedOneLevelLinks(
                attachment.id, ["RfTree", "LyTree", "HiTree"]
            )
            for child_id in children:
                attachment_child = self.env["ir.attachment"].browse(child_id)
                child_dict = self.singleItem(
                    attachment_child, level + 1, False, items_to_show, root_doc
                )
