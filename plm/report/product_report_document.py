...
class ReportProdStructure(models.AbstractModel):
        ...
        for product_id in product_ids:
            for attachment_id in product_id.linkeddocuments:
                if attachment_id.document_type.upper() == '3D':
                    ...
                    vals = self.singleItem(attachment_id, 
                                           0, 
                                           True, 
                                           items_to_show,
                                           attachment_id,
    ...
    @api.model
    def get_doc_prod(self, 
                     attachment, 
                     level=False, 
                     items_to_show={}, 
                     root_doc=False):
        data = self.singleItem(attachment, 
                               level, 
                               True, 
                               items_to_show, root_doc)
    ...
    def checkVisibleItems(self, 
        ...
        if not parent_doc.linkedcomponents:
            ...
            out.extend(parents)
        children = parent_doc.getRelatedOneLevelLinks(parent_doc.id, 
                                                      ['RfTree', 'HiTree'])
        for child_id in children:
            ...
            children_out = self.checkVisibleItems(attachment_child, 
                                                  new_parents)
    ...
    def singleItem(self, 
        ...
        if recursion and attachment.document_type.upper() == '3D':
            children = attachment.getRelatedOneLevelLinks(attachment.id, 
                                                          ['RfTree', 'LyTree', 'HiTree'])
            for child_id in children:
                attachment_child = self.env['ir.attachment'].browse(child_id)
                child_dict = self.singleItem(attachment_child, level + 1,
                                              False, 
                                              items_to_show, root_doc)
