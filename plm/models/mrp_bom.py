...
class MrpBomExtension(models.Model):
    ...
    def SaveStructure(self, relations, level=0, curr_level=0, kind_bom="normal"):
        ...
        def check_level(
            ...
            logging.info(
                "parent_id: {0}, source: {1}, child_id: {2}".format(
                    product_product_parent_id, source_ids, product_product_child_id
                )
            )
            evaluated = []
        ...
        def save_parent(name, part_id, source_id, kind_bom="normal"):
            ...
            try:
                ...
                raise AttributeError(
                    _(
                        "save_parent :  unable to create a relation for part ({0}) with source ({1}) : {2}.".format(
    ...
    def _bom_find_domain(
        ...
        domain = super(MrpBomExtension, self)._bom_find_domain(
            products, picking_type, company_id, bom_type
        )
        if not bom_type:
            available_types = ["engineering", "spare"]
            domain += [("type", "not in", available_types)]
        return domain
