...
class PlmComponentDocumentRel(models.Model):
    ...
    def SaveStructure(self, relations, level=0, currlevel=0):
        ...
        def cleanStructure(relations):
            ...
            for document_id, component_id in relations:
                ...
                prodProdObj.write(
                    {"linkeddocuments": [(3, document_id, False)]}
                )  # Clear link between component and document
        ...
        def saveChild(args):
            ...
            try:
                ...
                if compId and docId:
                    ...
                    compBrws.write(
                        {"linkeddocuments": [(4, docId, False)]}
                    )  # Update with existing id
            except Exception as ex:
                logging.warning(ex)
                logging.warning(
                    "saveChild : Unable to create a link. Arguments (%s)." % (str(args))
                )
    ...
    def createFromIds(self, product_product_id, ir_attachment_id):
        exsist = self.search_count(
            [
                ("component_id", "=", product_product_id.id),
