...
class MrpBomLineExtension(models.Model):
                ...
                num_boms = self.env["mrp.bom"].search_count(
                    [
                        ...
                        (
                            "type",
                            "in",
                            [bom_line.bom_id.type, "subcontract", "phantom"],
                        ),
                        ("active", "=", True),
                    ]
                )
            ...
            if line_brws.type == "normal":
                ...
                out_act_dict["view_ids"] = [
                    ...
                    (
                        ...
                        {
                            "view_mode": "list",
                            "view_id": self.env.ref("plm.plm_bom_list_view").id,
                        },
                    ),
                    (
                        0,
                        ...
                        {
                            "view_mode": "form",
                            "view_id": self.env.ref("plm.plm_bom_form_view").id,
                        },
                    ),
                ]
    ...
    def _related_doc_ids(self):
        for bom_line_brws in self:
            bom_line_brws.related_document_ids = (
                bom_line_brws.product_id.linkeddocuments
            )
    ...
    weight_net = fields.Float(
        related="product_id.weight", string="Weight Net", store=False
    )
    ...
    hasChildBoms = fields.Boolean(
        compute="_has_children_boms", string="Has Children Boms"
    )
    related_bom_ids = fields.One2many(
        compute="_related_boms",
        comodel_name="mrp.bom",
        string="Related BOMs",
        readonly=True,
    )
    related_document_ids = fields.One2many(
        compute="_related_doc_ids",
        comodel_name="ir.attachment",
        string="Related Documents",
    )
    cutted_type = fields.Selection(
        [("none", "None"), ("client", "Client"), ("server", "Server")],
        "Cutted Compute Type",
        default="none",
    )
