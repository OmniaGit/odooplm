...
class IrAttachment(models.Model):
    ...
    def canCheckOut1(self):
        for docBrws in self:
            if docBrws.isCheckedOutByMe():
                msg = _(
                    f"Unable to check-Out a document that is already checked Out By {docBrws.checkout_user}"
                )
                return docBrws.id, "check_out_by_me", msg
            if docBrws.is_checkout:
                msg = _(
                    f"Unable to check-Out a document that is already checked IN by user {docBrws.checkout_user}"
                )
                return docBrws.id, "check_out_by_user", msg
            if docBrws.engineering_state not in [START_STATUS, False]:
                msg = _(
                    f"Unable to check-Out a document that is in state {docBrws.engineering_state}"
                )
    ...
    def canCheckOut(self, showError=False):
        for docBrws in self:
            if docBrws.is_checkout:
                msg = _(
                    "Unable to check-Out a document that is already checked IN by user %r"
                    % docBrws.checkout_user
                )
                if showError:
                    raise UserError(msg)
                return False, msg
            if docBrws.engineering_state not in [START_STATUS, False]:
                msg = _(
                    "Unable to check-Out a document that is in state %r"
    ...
    def saveSingleLevel(self, clientArg):
        ...
        if not ir_attachment_id:
            logging.warning(
                "Unable to create / get ir_attachment from %s" % document_props
            )
        #  generate link
        if product_product_id and ir_attachment_id:
            ...
            logging.warning(
                "Unable to generate link from product: %s document: %s Thread %s"
    ...
    def createFromProps(
        ...
        if not engineering_code:
            raise UserError(
                "Unable to create document with empty name %r"
    ...
    def _preCheckInRecursive_all(self, root_id):
        ...
        if root_id.is3D():
            doc_3d_ids += root_id
            doc_3d_ids += self.browse(
                self.getRelatedHiTree(
                    root_id.id, recursion=True, getRftree=True, getOnyChkOut=True
                )
            )
        else:
            doc_2d_ids += root_id
            for doc_id in doc_3d_ids:
                doc_3d_ids += self.browse(
                    self.getRelatedHiTree(
                        doc_id.id, recursion=True, getRftree=True, getOnyChkOut=True
                    )
                )
        for doc_3d_id in doc_3d_ids:
            doc_2d_ids += self.browse(
                list(set(self.getRelatedLyTree(doc_3d_id.id, getOnyChkOut=True)))
            )
        done = []
        for s_doc_id in doc_3d_ids + doc_2d_ids:
            ...
            data_info, out_status = self.fill_up_check_in_status(
                s_doc_id, PLM_DT_DELTA, is_root=s_doc_id.id == root_id.id
            )
