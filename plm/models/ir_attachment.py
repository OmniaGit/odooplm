...
class IrAttachment(models.Model):
    ...
    preview = fields.Image(
        "Preview Content", max_width=1920, max_height=1920, attachment=False
    )

    checkout_user = fields.Char(string="Checked-Out to", compute="_get_checkout_state")

    is_checkout = fields.Boolean("Is Checked-Out", compute="_is_checkout", store=False)
    ...
    is_linkedcomponents = fields.Boolean(
        "Is Linked Components", compute="_compute_linkedcomponents"
    )
    ...
    is_library = fields.Boolean("Is Library file", default=False)
    library_path = fields.Char("File library path")

    must_update_from_cad = fields.Boolean(
        "Must Update form CAD",
    ...
    def actions(self):
        return {
            "reactivate": self.action_reactivate,
    ...
    def get_checkout_user(self):
        ...
        if lastDoc:
            for docBrws in self.env["plm.checkout"].search(
                [("documentid", "=", lastDoc[0])]
            ):
    ...
    def _get_last_rev_no_browser(self):
        ...
        for objDoc in self:
            doc_ids = self.search(
                [("engineering_code", "=", objDoc.engineering_code)],
            ...
            if not doc_ids:
                logging.warning(
                    '[_getlastrev] No documents are found for object with engineering_code: "%s"'
    ...
    def _data_get_files(
        self, listedFiles=([], []), forceFlag=False, local_server_name=True
    ):
        """
                ...
                if not (objDoc.name in listfiles):
                    ...
                    if forceFlag:
                        ...
                        timefile = time.mktime(
                            datetime.strptime(
                                str(datefiles[listfiles.index(objDoc.name)]),
                                "%Y-%m-%d %H:%M:%S",
                            ).timetuple()
                        )
                    ...
                    if isNewer and not (isCheckedOutToMe):
                        ...
                        result.append(
                            (objDoc.id, objDoc.name, False, isCheckedOutToMe, timeDoc)
                        )
            except Exception as ex:
                logging.error(
                    "_data_get_files : Unable to access to document ("
                    + str(objDoc.engineering_code)
                    + "). Error :"
                    + str(ex)
                )
                result.append(
                    (objDoc.id, objDoc.name, False, True, self.getServerTime())
                )
        if local_server_name != "odoo":
                ...
                if ir_attachment_id.is_plm and self.env.context.get("backup", True):
                    if (
                        ...
                        try:
                            ...
                            logging.warning(
                                "Directory %s olready present " % folder_name
                            )
                        to_file = os.path.join(folder_name, file_name)
                        shutil.copyfile(
                            self._full_path(ir_attachment_id.store_fname), to_file
                        )
                        self.env["plm.backupdoc"].create(
                            {
                                "userid": self.env.uid,
    ...
    def _explodedocs(self, oid, kinds, listed_documents=[], recursion=True):
        ...
        def getAllDocumentChildId(fromID, kinds):
            docRelBrwsList = documentRelation.search(
                [("parent_id", "=", fromID), ("link_kind", "in", kinds)]
            )
    ...
    def getRelatedOneLevelLinks(self, doc_id, kinds):
        ...
        for link_kind in kinds:
            if link_kind == "RfTree":
                ...
                logging.warning(
                    "getRelatedOneLevelLinks cannot find link_kind %r" % (link_kind)
                )
        return list(set(result))

    @api.model
    def getRelatedLyTree(self, doc_id, optional_return_type=["3d"], getOnyChkOut=False):
        out = []
        if not doc_id:
        ...
        for doc_rel_id in doc_rel_ids:
            if parent_doc_type == "3d":
                if (
                    doc_rel_id.parent_id.id == doc_id
                    and doc_rel_id.child_id.document_type == "2d"
                ):
                    out.append(doc_rel_id.child_id.id)
                elif (
                    doc_rel_id.child_id.id == doc_id
                    and doc_rel_id.parent_id.document_type == "2d"
                ):
                    out.append(doc_rel_id.parent_id.id)
            elif parent_doc_type == "2d":
                if (
                    doc_rel_id.parent_id.id == doc_id
                    and doc_rel_id.child_id.document_type in optional_return_type
                ):
                    out.append(doc_rel_id.child_id.id)
                elif (
                    doc_rel_id.child_id.id == doc_id
                    and doc_rel_id.parent_id.document_type in optional_return_type
                ):
                    out.append(doc_rel_id.parent_id.id)
        return list(set(out))

    @api.model
    def getRelatedPrTree(self, root_doc_id, recursion=False, getOnyChkOut=False):
        out = []
        ...
        def _getRelatedPrTree(doc_id):
            ...
            for doc_rel_id in doc_rel_ids:
                ...
                if doc_type in ["3d", "2d"]:
                    if (
                        doc_rel_id.parent_id.id == doc_id
                        and doc_rel_id.child_id.document_type == "pr"
                    ):
                        good_id = doc_rel_id.child_id
                    if (
                        doc_rel_id.child_id.id == doc_id
                        and doc_rel_id.parent_id.document_type == "pr"
                    ):
                        good_id = doc_rel_id.parent_id
                elif doc_type == "pr":
                    if (
                        doc_rel_id.parent_id.id == doc_id
                        and doc_rel_id.child_id.document_type == "3d"
                    ):
                        good_id = doc_rel_id.child_id
                    elif (
                        doc_rel_id.child_id.id == doc_id
                        and doc_rel_id.parent_id.document_type == "3d"
                    ):
                        good_id = doc_rel_id.parent_id
    ...
    def getRelatedRfTree(self, doc_id, recursion=True, evaluated=False):
        ...
        if doc_id in evaluated:
            logging.warning(
                "Document %r already found in RfTree evaluated %r" % (doc_id, evaluated)
            )
            return out
        ...
        for doc_rel_id in doc_rel_ids:
            if doc_rel_id.child_id.id == doc_id:
                ...
                if recursion:
                    out.extend(
                        self.getRelatedRfTree(
                            doc_rel_id.parent_id.id, recursion, evaluated
                        )
                    )
            else:
                out.append(doc_rel_id.child_id.id)
                if recursion:
                    out.extend(
                        self.getRelatedRfTree(
                            doc_rel_id.child_id.id, recursion, evaluated
                        )
                    )
        return list(set(out))
    ...
    def getRelatedPkgTree(self, doc_id):
        ...
        if not doc_id:
            logging.warning("Cannot get links from %r document" % (doc_id))
            return []
        to_search = [("link_kind", "in", ["PkgTree"]), ("parent_id", "=", doc_id)]
        doc_rel_ids = self.env["ir.attachment.relation"].search(to_search)
        for doc_rel_id in doc_rel_ids:
            out.append(doc_rel_id.child_id.id)
        return list(set(out))
    ...
    @api.model
    def getRelatedHiTree(
        self, doc_id, recursion=True, getRftree=False, getOnyChkOut=False
    ):
        """
        ...
        def _getRelatedHiTree(doc_id, recursion, getRftree):
            ...
            children_attachment_ids = self.env["ir.attachment.relation"].search(
                [("link_kind", "=", "HiTree"), ("parent_id", "=", doc_id)]
            )
    ...
    def computeDownloadStatus(self, hostname, pws_path, latest=False):
        """
        ...
        for ir_attachment_id in self:
            ...
            if not isCheckedOutToMe:
                is_collectable = ir_attachment_id.isCollectable(hostname, pws_path)
            #
        ...
        for objDoc in self.browse(targetIds):
            ...
            if datas_fname in listfiles:
                if forceFlag:
                    ...
                    isNewer = objDoc.checkNewer(hostname, hostpws)
    ...
    def _iswritable(self, oid):
        if not oid.type == "binary":
            logging.warning(
                ...
                + str(oid.name)
                + "-"
                + str(oid.engineering_revision)
                + ") not writable as hyperlink."
            )
            return False
            ...
            for componentBrws in self.linkedcomponents:
                for bomBrws in componentBrws.bom_ids:
                    if bomBrws.source_id.id == tmpObject.id:
                        ...
                        logging.info(
                            "Bom ID %r update with new source ID %r / %r"
                            % (bomBrws.id, tmpObject.id, newObj.id)
                        )
    ...
    def CheckSaveUpdate(self, documents, default=None):
        ...
        for document in documents:
            hasToBeSaved = False
            if not ("engineering_code" in document) or (
                "engineering_revision" not in document
            ):
    ...
    def CleanUp(self, default=None):
        ...
        """
        self.env.cr.execute(
            "delete from ir_attachment where store_fname=NULL and type='binary'"
        )
        return True
    ...
    def ischecked_in(self):
        ...
        for document in self:
            if checkoutType.search([("documentid", "=", document.id)]):
                logging.warning(
                    _(
                        "The document %s - %s has not checked-in"
                        % (
                            str(document.engineering_code),
                            str(document.engineering_revision),
                        )
                    )
                )
                return False
    ...
    def _get_filestore(self):
        ...
        try:
            ...
            if ex.errno == 13:
                logging.warning(
                    _("Permission denied for folder %r." % (str(filestore)))
                )
                return ""
        return filestore
    ...
    def create(self, vals):
        ...
        res = super().create(to_create_vals)
        res.with_context(create=True).check_unique()
        return res

    def update_component_preview(self, product_id=False):
        for ir_attachment_id in self:
            if ir_attachment_id.document_type == "3d" and ir_attachment_id.preview:
                ...
                for product_product_id in ir_attachment_id.linkedcomponents:
                    to_update[product_product_id.engineering_revision] = (
                        product_product_id
                    )
                if to_update:
    ...
    def readMany2oneFields(self, readVals, fields):
        return self.env["product.product"]._readMany2oneFields(
            self.env["ir.attachment"], readVals, fields
        )

    def checkMany2oneClient(self, vals):
        return self.env["product.product"]._checkMany2oneClient(
            self.env["ir.attachment"], vals
        )
    ...
    def is_plm_state_writable(self):
        for customObject in self:
            if customObject.engineering_state in PLM_NO_WRITE_STATE:
                logging.info(
                    "state %r not in %r"
                    % (customObject.engineering_state, PLM_NO_WRITE_STATE)
                )
                return False
        return True
            ...
            for attachment_id in self:
                if not (self.env.user._is_admin() or self.env.user._is_superuser()):
                    if (
                        ...
                        and attachment_id.document_type.upper() in ["2D", "3D"]
                    ):
                        if not attachment_id._is_checkedout_for_me():
                            raise UserError(
                                _(
                                    "You cannot edit a file not in check-out by you! User ID %s"
                                    % (self.env.uid)
                                )
                            )
                        if not attachment_id.is_plm_state_writable():
                            raise UserError(
                                _(
                                    "The active state does not allow you to make save action"
                                )
                            )
    ...
    def getParentDocuments(self):
        ...
        for doc in self:
            ...
            att_rel_obj = self.env["ir.attachment.relation"].search(
                [("child_id", "=", doc.id)]
            )
            ...
            for child_doc, parent_docs in id_parents.items():
                if parent_docs:
                    ...
                    for parent_doc in parent_docs:
                        msg += _(
                            f"\t Engineering Name = {parent_doc.engineering_code}   Engineering Revision = {parent_doc.engineering_revision}   Id = {parent_doc.id}\n"
                        )
                    raise UserError(msg)
            #
            # Check Bom relations
        ...
        for checkObj in self:
            ...
            for oldObject in docBrwsList:
                ...
                if not oldObject.with_context(check=False).write(values):
                    msg = (
                        "Unlink : Unable to update state in old document Engineering Name = %r   Engineering Revision = %r   Id = %r"
                        % (
                    ...
                    raise UserError(
                        _(
                            "Cannot restore previous document Engineering Name = %r   Engineering Revision = %r   Id = %r"
                            % (
                                oldObject.engineering_code,
                                oldObject.engineering_revision,
                                oldObject.id,
    ...
    def unlinkBackUp(self):
        for checkObj in self:
            docBrwsList = self.env["plm.backupdoc"].search(
                [("documentid", "=", checkObj.id)]
            )
            for oldObject in docBrwsList:
    ...
    def _check_duplication(self, vals, ids=None, op="create"):
        ...
        if op == "write":
            for ir_attachment_id in self.browse(ids):
                ...
                if not parent_id:
                    parent_id = (
                        ir_attachment_id.parent_id
                        and ir_attachment_id.parent_id.id
                        or False
                    )
                # TODO fix algo
                if not ressource_parent_type_id:
                    ressource_parent_type_id = (
                        ir_attachment_id.ressource_parent_type_id
                        and ir_attachment_id.ressource_parent_type_id.id
                        or False
                    )
                if not ressource_id:
                    ressource_id = (
                        ir_attachment_id.ressource_id
                        and ir_attachment_id.ressource_id
                        or 0
                    )
                docBrwsList = self.search(
                    [
    ...
    def _check_in(self):
        ...
        if not checkOutId:
            msg = (
                """"Document %r is not in check out by user %r so cannot be checked-in"""
                % (self.id, self.env.user.id)
            )
            logging.info(msg)
            return False
        if self.file_size <= 0 or not self.name:
            msg = "Document %r has not document content so cannot be checked-in" % (
                self.id
            )
            logging.warning(msg)
            return False
    ...
    def assign_must_update_flag(self, only_layout=False):
            ...
            if plm_cad_open:
                ...
                if attachment_id.document_type == "3d":
                    for layout_attachment_id in attachment_id.getRelatedLayouts():
                        child_plm_cad_open = cad_open_obj.getLastCadSave(
                            layout_attachment_id
                        )
                        child_db_thread = child_plm_cad_open.dbThread
                        if db_thread not in ["", False] and child_db_thread not in [
                            "",
                            False,
                        ]:
                    ...
                    for ref_attachment_id in attachment_id.getRelatedModels():
                        ...
                        if (
                            child_plm_cad_open
                            and child_plm_cad_open.write_date < source_date
                        ):
                            attachment_id.must_update_from_cad = False
                        else:
    ...
    def getRelatedModels(self):
        ...
        for ir_attachment_id in self:
            for relation in ir_attachment_relation.search(
                [("parent_id", "=", ir_attachment_id.id)]
            ):
    ...
    def _compute_document_type(self):
        ...
        for docBrws in self:
            try:
                ...
                logging.error(
                    "Unable to compute document type for document %r, error %r"
                    % (docBrws.id, ex)
                )
    ...
    def _getHtmlDocument(self):
        ...
        html_rendered = view_obj._render_template(
            "plm.document_search_button", {"doc": self}
        )
        html_tooltip = view_obj._render_template(
            "plm.document_search_tooltip", {"doc": self}
        )
    ...
    def getAttachedHtmlDoucment(self, product_ids):
        ...
        for product_id in self.env["product.product"].browse(product_ids):
            for ir_attachment in product_id.linkeddocuments:
                ...
                out.append(
                    (ir_attachment.id, product_id.id, html_rendered, html_tooltip)
                )
        return out
    ...
    def _attachment_revision_count(self):
        ...
        for ir_attachment_id in self:
            if ir_attachment_id.engineering_code is not False:
                ir_attachment_id.attachment_revision_count = (
                    ir_attachment_id.search_count(
                        [("engineering_code", "=", ir_attachment_id.engineering_code)]
                    )
                )
            else:
    ...
    def CheckedIn(self, files, default=None):
        ...
        def getcheckedfiles(files):
            ...
            for fileName in files:
                plmDocList = self.search(
                    [("name", "ilike", fileName)], order="engineering_revision DESC"
                )
                if len(plmDocList) > 0:
                    ids = plmDocList.ids
                    ids.sort()
    ...
    def GetLatestIds(self, vals, forceCADProperties=False):
        ...
        def getCompIds(docName, docRev):
            if docRev is None or docRev is False:
                docBrwsList = self.search(
                    [("engineering_code", "=", docName)], order="engineering_revision"
                )
                if len(docBrwsList) > 0:
    ...
    def isCheckedOutByMeOr(self, host_name, host_pws):
        checkoutBrwsList = self.env["plm.checkout"].search(
            [
                ("documentid", "=", self.id),
                ("userid", "=", self.env.uid),
    ...
    def CheckInRecursive(self, request, default=None):
        ...
        def getDocId(args):
            ...
            for docBrwsList in self.search(
                [
                    ("engineering_code", "=", engineering_code),
                    ("engineering_revision", "=", docRev),
                ]
            ...
            logging.warning(
                'Document with engineering_code "%s" and revision "%s" not found'
                % (engineering_code, docRev)
            )
            return False
        ...
        if not docBrws.isCheckedOutByMe():
            logging.info(
                "Document %r is not in check out by user %r so cannot be checked-in recursively"
                % (oid, self.env.uid)
            )
            return False
        if docBrws.file_size <= 0 or not docBrws.name:
            logging.warning(
                "Document %r has not document content so cannot be checked-in recirsively"
                % (oid)
            )
            return False
        if selection is False:
        ...
        for docId in docArray:
            checkOutBrwsList = checkoutObj.search(
    ...
    def GetRelatedDocs(self, default=None, getBrowse=False):
        ...
        for document in self.browse(read_docs).sorted(
            "document_type", reverse=True
        ):  # 3d before 2d
            if getBrowse:
    ...
    def getCheckedOut(self, oid, default=None):
        ...
        for checkOutBrws in checkoutBrwsList:
            return (
                checkOutBrws.documentid.engineering_code,
    ...
    def _file_delete(self, fname):
        ...
        """
        backupDocBrwsList = self.env["plm.backupdoc"].search(
            [("existingfile", "=", fname)]
        )
    ...
    def GetNextDocumentName(self, documentName):
        ...
        eng_code = ctx.get("product_attrs", {}).get("engineering_code") or ctx.get(
            "engineering_code", ""
        )
        #
    ...
    def canBeSavedClient(self, documentValues={}, returnCode=False):
        ...
        if returnCode:
            return (
                True,
                _("Document %r with revision %r not present in Odoo.")
                % (engineering_code, docRev),
                "NO_ERROR",
            )
        return True, _("Document %r with revision %r not present in Odoo.") % (
            engineering_code,
            docRev,
        )
        ...
        if not skipCheckOutControl:
            ...
            if checkOutObject:
                if checkOutObject.userid.id != self.env.uid:
                    outMessage = _(
                        "Document is Check-Out from User %r", checkOutObject.name
                    )
    ...
    def saveStructure(self, arguments):
            ...
            try:
                ...
                for brwItem in self.search(
                    ...
                    if brwItem.id in alreadyEvaluated:
                        ...
                        documentAttribute["TO_UPDATE"] = (
                            False  # To skip same document preview/pdf uploading by the client
                        )
                        break
                    if brwItem.engineering_state not in [
                        RELEASED_STATUS,
                        OBSOLATED_STATUS,
                    ]:
            ...
            try:
                ...
                if not prodBrws:
                    if not productAttribute.get("name", False):
                        productAttribute["name"] = productAttribute.get(
                            "engineering_code", False
                        )
                    if not productAttribute.get(
                        "engineering_code", False
                    ):  # I could have a document without component, so not create product
                        continue
        ...
        for parentId, childrenRelations in list(documentRelations.items()):
            try:
                ...
                for objBrw in documentRelationTemplate.search(
                    [("parent_id", "=", trueParentId)]
                ):
                    found = False
                    for childId, relationType in childrenRelations:
                        trueChildId = documentAttributes.get(childId, {}).get("id", 0)
                        if (
                            objBrw.parent_id.id == trueParentId
                            and objBrw.child_id.id == trueChildId
                            and objBrw.link_kind == relationType
                        ):
                            found = True
                ...
                for childId, relationType in childrenRelations:
                    ...
                    if key in createdDocRels:
                        continue
                    createdDocRels.append(key)
                    documentRelationTemplate.create(
                        {
                            "parent_id": trueParentId,
        ...
        for parentId, childRelations in list(productRelations.items()):
            try:
                ...
                brwBoml = mrpBomTemplate.search(
                    [("product_tmpl_id", "=", productTempId)]
                )
                if not brwBoml:
                    brwBoml = mrpBomTemplate.create(
                        {"product_tmpl_id": productTempId, "type": bomType}
                    )
                # delete rows
                if skipDocumentCheckOnBom:
                    ...
                    for _, documentId, _ in childRelations:
                        trueDocumentId = documentAttributes.get(documentId, {}).get(
                            "id", 0
                        )
                        if (
                            trueDocumentId
                        ):  # seems strange this .. but i will delete only the bom with the right source id
                            for brwBom in brwBoml:
                ...
                for childId, documentId, relationAttributes in childRelations:
                    if skipDocumentCheckOnBom:
                        if not childId:
                            logging.warning(
                                "Bad relation request %r, %r" % (childId, documentId)
                            )
                            continue
                    ...
                    brwBoml.add_child_row(
                        trueChildId, trueDocumentId, relationAttributes, bomType
                    )
            except Exception as ex:
                logging.error(ex)
                raise ex
    ...
    def clientCanCheckOut(self, doc_attrs):
        for attachment_id in self.getDocumentBrws(doc_attrs):
            return attachment_id.canCheckOut1(
                doc_attrs[0]["HOST_NAME"], doc_attrs[0]["HOST_PWS"]
            )
        return False, "not_found", f"File Not found from attributes {doc_attrs}"

    def canCheckOut1(self, host_name="", host_pws=""):
        for docBrws in self:
            if docBrws.engineering_state not in [START_STATUS, False]:
                msg = _(
                    f"Unable to check-Out document {docBrws.name} that is in state {docBrws.engineering_state}"
                )
                return docBrws.id, "check_out_released", msg
            check_out, check_out_browser = docBrws.isCheckedOutByMeOr(
                host_name, host_pws
            )
            if check_out:
                msg = _(
                    f"Unable to check-Out document {docBrws.name} that is already checked Out By {docBrws.checkout_user}"
                )
                return docBrws.id, "check_out_by_me", msg
            else:
                if check_out_browser:
                    msg = _(
                        f"Unable to check-Out document {docBrws.name} that is already checked out by user {docBrws.checkout_user} on {host_name}, {host_pws}"
                    )
    ...
    def canCheckOut(self, showError=False):
        for docBrws in self:
            if docBrws.is_checkout:
                msg = _(
                    f"Unable to check-Out document {docBrws.name} that is already checked IN by user {docBrws.checkout_user}"
                )
                if showError:
                    raise UserError(msg)
                return False, msg
    ...
    def needUpdate(self):
        ...
        """
        checkoutIds = self.env["plm.checkout"].search(
            [("documentid", "=", self.id), ("userid", "=", self.env.uid)]
        )
    ...
    def getCloneRevisionStructure(self, values=[]):
        ...
        if not engineering_code or docRev is None:
            logging.warning(
                "No name or not revision passed by the client %r" % (docProps)
            )
            return {}
        docBrws = self.search(
            [
    ...
    def computeLikedDocuments(self):
        ...
        for linkedBrws in linkedDocEnv.search([("child_id", "=", self.id)]):
            if (
                linkedBrws.parent_id.document_type.upper() == "2D"
            ):  # Append only 2D relations
                docList.append(
                    {
                        "component": {},
                        "document": linkedBrws.parent_id.getDocumentInfos(),
                    }
                )
        return docList
    ...
    def cleanDocumentRelations(self):
        linkedDocEnv = self.env["ir.attachment.relation"]
        for docBrws in self:
            for linkedBrws in linkedDocEnv.search(
                [("child_id", "=", docBrws.id), ("parent_id", "=", docBrws.id)]
            ):
                linkedBrws.unlink()
    ...
    def checkStructureDocument(self, docAttrs):
        ...
        docBrwsList = self.search(
            [("engineering_code", "=", docName)], order="engineering_revision DESC"
        )
        existingDocs = {}
        graterDocBrws = None
        matchDocBrws = None
        ...
        for docBrws in docBrwsList:
            ...
            existingDocs[docBrws.engineering_revision] = (
                docBrws.engineering_code,
                docBrws.engineering_state,
            )
    ...
    def checkStructureComponent(self, compAttrs):
        ...
        prodBrwsList = prodProd.search(
            [("engineering_code", "=", engCode)], order="engineering_revision DESC"
        )
        ...
        for compBrws in prodBrwsList:
            ...
            existingCompRevisions[compBrws.engineering_revision] = (
                compBrws.engineering_code,
                compBrws.engineering_state,
            )
        ...
        def getLinkedDocumentsByComponent(node, compBrws):
            ...
            if compBrws.id:
                ...
                for docBrws in compBrws.linkeddocuments:
                    ...
                    newNode["DOCUMENT_ATTRIBUTES"][
                        "CAN_BE_REVISED"
                    ] = docBrws.canBeRevised()
        ...
        def getLinkedDocumentsByDocument(node, docBrws):
            ...
            if docBrws.id:
                ...
                for docLinkBrws in linkedDocEnv.search([(typeDoc, "=", docBrws.id)]):
                    ...
                    if (
                        relatedDocBrws and relatedDocBrws.document_type.upper() == "2D"
                    ):  # If not 2D is a raw part
                        newNode = copy.deepcopy(node)
                        newNode["DOCUMENT_ATTRIBUTES"] = (
                            relatedDocBrws.getDocumentInfos()
                        )
                        newNode["DOCUMENT_ATTRIBUTES"][
                            "CAN_BE_REVISED"
                        ] = relatedDocBrws.canBeRevised()
                        node["RELATIONS"].append(newNode)
        ...
        def recursionUpdate(node, isRoot=False):
            ...
            if isRoot:
                node["PRODUCT_ATTRIBUTES"][
                    "CAN_BE_REVISED"
                ] = True  # Has already been revised
                engcode = compVals.get("engineering_code")
                if (
                    not engcode
                ):  # Has already been revised and contains only document props
            ...
            if compId:
                ...
                getLinkedDocumentsByDocument(
                    node, rootDocBrws
                )  # Only for document infos
    ...
    def getCheckedOutAttrs(self, vals):
        ...
        for fileDict in attrsList:
            ...
            if not docBrwsList:
                outLocalDict["help_checkout"] = _(
                    "Unable to find document to check-out. Please save it."
                )
            else:
                for docBrws in docBrwsList:
    ...
    def saveSingleLevel(self, clientArg):
        ...
        if not ir_attachment_id:
            logging.warning(
                f"Unable to create / get ir_attachment from {document_props}"
            )
        #
        #  Generate link
        #
        if product_product_id and ir_attachment_id:
            self.env["plm.component.document.rel"].createFromIds(
                product_product_id, ir_attachment_id
            )
        else:
            logging.warning(
                f"Unable to generate link from product: {product_product_id} document: {ir_attachment_id} Thread {dbThread}"
            )
        #
        # Out
        #
    ...
    @api.model
    def createFromProps(
        self, documentAttribute={}, dbThread=False, hostName=False, hostPws=False
    ):
        action = "upload"
        if (
            documentAttribute.get("CUTTED_COMP", False)
            or documentAttribute.get("VIRTUAL", False)
            or documentAttribute.get("ONLY_COMP", False)
        ):
            return False, "jump"
        engineering_code = documentAttribute.get("engineering_code", False)
        if not engineering_code:
            raise UserError(
                _("Unable to create document with empty name %r")
                % (documentAttribute.get("KEY", ""))
            )
        ...
        for seached_ir_attachemnt_id in self.search(
            [
                ("engineering_code", "=", engineering_code),
                (
                    "engineering_revision",
        ...
        #
        if found:
            if ir_attachemnt_id.engineering_state not in [
                RELEASED_STATUS,
                OBSOLATED_STATUS,
            ]:
        ...
        #
            ...
            documentAttribute["first_source_path"] = documentAttribute.get(
                "INTEGRATION_ORIG_FILE_PATH", ""
            )
    ...
    def getLastBackupDoc(self):
        for document_id in self:
            return self.env["plm.backupdoc"].search(
                [("documentid", "=", document_id.id)], order="create_date DESC", limit=1
            )
        return False

    def setupCadOpen(self, hostname="", pws_path="", operation_type="", dbthread=""):
        plm_cad_open = self.env["plm.cad.open"].sudo()
        if hostname and pws_path:
            for doc_id in self:
                last_bck = doc_id.getLastBackupDoc()
                plm_cad_open_brws = plm_cad_open.search(
    ...
    def canIUpload(self, dbTheread):
        ...
        for ir_attachment_id in self:
            key = f"{ir_attachment_id.engineering_code}_{ir_attachment_id.engineering_revision}"
            threadCodelist = plm_dbthread.search(
                [("documement_name_version", "=", key), ("done", "=", False)]
            ).mapped(lambda x: int(x.threadCode))
    ...
    def isDownloadableFromServer(self, args):
        ...
        if selection == 2:
            ...
            docArray = ids
        return self.browse(docArray)._data_get_files(
            listedFiles, forceFlag, _local_server_name
        )
    ...
    def GetProductDocumentId(self, clientArgs):
        ...
        return (
            False if not product_product_id else product_product_id.id,
    ...
    def _GetproductDocumentID(self, clientArgs):
        ...
        if engineering_code:
            for product_product in self.env["product.product"].search(
                [
                    ("engineering_code", "=", engineering_code),
                    ("engineering_revision", "=", engineering_revision),
                ]
        ...
        if document_name:
            for plm_document in self.env["ir.attachment"].search(
                [
                    ("engineering_code", "=", document_name),
    ...
    def checkNewer(self, hostname="", hostpws=""):
        self.ensure_one()
        for document in self:
            plm_cad_open = self.sudo().env["plm.cad.open"].getLastCadSave(document)
            if (
                plm_cad_open.hostname
                and plm_cad_open.hostname.lower() != hostname.lower()
            ):
                return True
            if (
                plm_cad_open.pws_path
                and plm_cad_open.pws_path.lower() != hostpws.lower()
            ):
                return True
    ...
    def getDocId(self, args):
        ...
        logging.warning(
            'Document with name "%s" and revision "%s" not found' % (docName, docRev)
        )
        return False

    @api.model
    def CheckIn2(self, request, default=None, force=False):
        if self.CheckInRecursive2(
            request[0], default=default, force=force, recursive=False
        ):
            return [request[0]]
        return []
    ...
    def CheckInRecursive2(self, involved_docs_dict, **kargs):
        ...
        for doc_vals in involved_docs_dict.get("to_check_in", []):
            ...
            if not docId:
                raise UserError(
                    f"Cannot check-in document with id False. Vals {doc_vals}"
                )
    ...
    def _bulk_prefetch_check_in_data(self, doc_ids):
        ...
        doc_data = self.browse(doc_ids).read(
            [
                ...
                "name",
                "document_type",
                "must_update_from_cad",
                "write_date",
            ]
        )
        unique_codes = list(
            {d["engineering_code"] for d in doc_data if d["engineering_code"]}
        )
        latest_id_by_code = {}
        if unique_codes:
    ...
    def getDefaulValueDict(
        self, docBrws, PLM_DT_DELTA, is_root, timing=None, prefetch=None
    ):
        tmp_dict = {}
        ...
        if prefetch is not None:
            ...
            tmp_dict["write_date"] = effective_date.strftime(
                DEFAULT_SERVER_DATETIME_FORMAT
            )
        else:
            tmp_dict["write_date"] = docBrws.getLastCadSave().strftime(
                DEFAULT_SERVER_DATETIME_FORMAT
            )
        ...
        _t = time.perf_counter()
        if prefetch is not None:
            tmp_dict["check_out_by_me"] = (
                docBrws.id in prefetch["checked_out_by_me_ids"]
            )
        else:
    ...
    def fill_up_check_in_status(
        self, docBrws, PLM_DT_DELTA, is_root, timing=None, prefetch=None
    ):
        out_status = ""
        data_info = self.getDefaulValueDict(
            docBrws, PLM_DT_DELTA, is_root, timing=timing, prefetch=prefetch
        )
        ...
        if is_checkout:
            ...
            if is_checked_out_by_me:
                data_info["options"] = {
                    "save_and_check_in": _("Save and check-in"),
    ...
    def _batch_traverse_hi_ly_tree(self, root_doc):
        ...
        if candidate_2d:
            ...
            all_2d_ids = [
                d["id"] for d in doc_data if d["document_type"] in ("2d", "pr")
            ]
    ...
    @api.model
    def preCheckInRecursive_all(self, doc_props):
        """
    ...
    def _preCheckInRecursive_all(self, root_id):
        out = {"to_check_2d": [], "to_check_3d": [], "info": [], "root_ent": False}
        PLM_DT_DELTA = self.getPlmDTDelta()
        if not root_id.isCheckedOutByMe():
            data_info, _out_status = self.fill_up_check_in_status(
                root_id, PLM_DT_DELTA, is_root=True
            )
        ...
        if root_id.is3D():
            ...
            initial_ids = list(
                set(
                    ...
                    + self.getRelatedPrTree(
                        root_id.id, recursion=True, getOnyChkOut=True
                    )
                )
            )
    ...
    def preCheckInRecursive(
        ...
        out = {  # pylint: disable=unreachable
            "to_check_in": [],
            "to_ask": [],
            "to_block": [],
            "to_info": [],
        ...
        def setupInfos(out, docBrws, PLM_DT_DELTA, is_root, doc_dict_3d=False):
            ...
            tmp_dict["name"] = docBrws.name
            tmp_dict["document_type"] = docBrws.document_type.upper()
            tmp_dict["write_date"] = docBrws.write_date.strftime(
                DEFAULT_SERVER_DATETIME_FORMAT
            )
            tmp_dict["check_in"] = docBrws.ischecked_in()
            tmp_dict["check_out_by_me"] = docBrws.isCheckedOutByMe()
            ...
            tmp_dict["msg"] = ""
            if doc_dict_3d:
                if doc_dict_3d["check_in"] or (
                    not doc_dict_3d["check_in"] and not doc_dict_3d["check_out_by_me"]
                ):
                    if doc_dict_3d["plm_cad_open_newer"]:
                        doc_dict_3d["msg"] = (
                            "Model %r related to drawing %r is not updated."
                            % (doc_dict_3d["name"], tmp_dict["name"])
                        )
                        appendItem(out["to_block"], doc_dict_3d)
                elif forceCheckInModelByDrawing:
                    msg = "Model %r related to drawing %r " % (
                        doc_dict_3d["name"],
                        tmp_dict["name"],
                    )
                    if doc_dict_3d["check_in"]:
                        msg += "is Checked in"
                    if (
                        not doc_dict_3d["check_in"]
                        and not doc_dict_3d["check_out_by_me"]
                    ):
                    ...
                    if doc_dict_3d["plm_cad_open_newer"]:
                        msg += (
                            "Cad date not aligned %r "
                            % doc_dict_3d["plm_cad_open_newer"]
                        )
                    doc_dict_3d["msg"] = msg
                    appendItem(out["to_block"], doc_dict_3d)
            ...
            if is_root:
                if tmp_dict["check_in"]:
                    if tmp_dict["plm_cad_open_newer"]:
                        tmp_dict["msg"] = (
                            "Document %r already checked-in but not updated."
                            % (tmp_dict["name"])
                        )
                    ...
                    tmp_dict["msg"] = (
                        "Document %r is in check-out by another user. Cannot check-in."
                        % (tmp_dict["name"])
                    )
                ...
                if tmp_dict["check_in"]:
                    ...
                    tmp_dict["msg"] = (
                        "Document %r is in check-out by another user. Cannot check-in, skipped."
                        % (tmp_dict["name"])
                    )
                    appendItem(out["to_info"], tmp_dict)
                    if tmp_dict["plm_cad_open_newer"]:
        ...
        def recursionf(
            ...
            for doc3D in docs3D:
                ...
                doc3DChildrens = self.browse(
                    self.getRelatedHiTree(doc3D.id, recursion=False)
                )
                ...
                docsReference = self.browse(
                    self.getRelatedRfTree(doc3D.id, recursion=False)
                )
            ...
            for doc_id in document_ids:
                ...
                if checkout:
                    if isCheckoutByMe:
                        ...
                        continue
                    if not stop:
                        checkout_id, msg = doc_id.checkout(
                            hostname, pws_path, showError=False
                        )
                        if not checkout_id or msg:
                            doc_fields["err_msg"] += "\n%s" % (msg)
                        else:
                            doc_id.setupCadOpen(
                                hostname, pws_path, operation_type="check-out"
                            )
                            doc_fields["checkout"] = True
                    else:
                        doc_fields["err_msg"] += "\nCannot checkout document %s." % (
                            doc_fields["name"]
                        )
    ...
    def getDocBom(self, level=0, recursion=True, report_obj=None):
        for attachment_id in self:
            children_list = []
            if recursion and attachment_id.document_type.upper() not in ["2D"]:
                children = attachment_id.getRelatedOneLevelLinks(
                    attachment_id.id, ["RfTree", "LyTree", "HiTree"]
                )
                for child_id in children:
                    attachment_child = self.browse(child_id)
                    child_dict = attachment_child.getDocBom(
                        level + 1, recursion=False, report_obj=report_obj
                    )
    ...
    def getDocBomFlat(self, latest=False):
        """
        ...
        #
        def get_all_ids(attachment_id, link_kinds, latest):
            out = []
            check = []
            ...
            def _get_all_ids(attachment_id):
                ...
                out.append(root_id)
                for root_model_attachment_id in root_id.getRelatedOneLevelLinks(
                    root_id.id, link_kinds
                ):
        ...
        if self.document_type.upper() in ["2D"]:
            out = []
            check = []
            for attachment_id in get_all_ids(self, ["LyTree", "RfTree"], latest):
                if attachment_id.id not in check:
                    ...
                    if attachment_id.document_type.upper() in ["3D"]:
                        for model_attachment_id in get_all_ids(
                            attachment_id, ["HiTree", "RfTree"], latest
                        ):
    ...
    def getDocBomFlatSql(self):
        ...
        for link_kind in to_compute:
            for related_attachment_id in self.get_all_relation_flat_structure_sql(
                link_kind
            ):
    ...
    @api.model
    def sent_check_out_requests(self, document_id, host_name, host_pws):
        """
        create an activity on document asking to check-out the document
        """
        for ir_attachment_id in self.browse([document_id]):
            _id, action, _message = ir_attachment_id.canCheckOut1(host_name, host_pws)
    ...
    def open_related_dbthread(self):
        ...
        for ir_attachment_id in self:
            ...
            plm_dbthread_ids = plm_dbthread.search(
                [("documement_name_version", "=", search_name)]
            )
        #
        return {
            "name": _("Saving Error"),
    ...
    def fillUpClonedStructure(self, args):
        ...
        def _recursion(cad_structure):
            ...
            out_parent_attrs = self.get_clone_info_attr(
                attachment_id, product_product_id
            )
            out_parent_attrs["CONFIGURATION_NAME"] = parent_attrs.get(
                "product", {}
            ).get("CONFIGURATION_NAME", "")
            out_parent_attrs["CONFIGURATION_NAME"] = parent_attrs.get(
                "product", {}
            ).get("CONFIGURATION_NAME", "")
            out_parent_attrs["INTEGRATION_FILE_TYPE"] = parent_attrs.get(
                "document", {}
            ).get("INTEGRATION_FILE_TYPE", "")
            out_parent_attrs["CONFIGURATIONS_ATTRIBUTES"] = {}
            for config_name, config_attrs in (
                parent_attrs.get("product", {})
                .get("CONFIGURATIONS_ATTRIBUTES", {})
                .items()
            ):
                config_product_product_id, config_attachment_id = (
                    self._GetproductDocumentID(tuple(parent_attrs.values()))
                )
                config_out_parent_attrs = self.get_clone_info_attr(
                    config_attachment_id, config_product_product_id
                )
                out_parent_attrs["CONFIGURATIONS_ATTRIBUTES"][config_name] = (
                    config_out_parent_attrs.get("product")
                )
            #
    ...
    def get_clone_info_attr(self, doc_id, product_product_id=None):
    ...
    @api.model
    def getCloneStructure(self, args):
        #
        logging.warning(
            "Function getClonedStructure will be removed in the new verison !!"
        )
        #
        ...
        product_product_id, attachment_id = self._GetproductDocumentID(
            tuple(main_root_attributes.values())
        )
        #
        if attachment_id:
            #
            doc_ids_2d = []
            #
            main_parent_attrs = get_clone_info_attr(
                attachment_id.id, product_product_id
            )
            out["MAIN"] = main_parent_attrs
            for k in SUPPORT_MAIN_PRODUCT_ATTRIBUTES:
                out["MAIN"]["product"][k] = main_root_attributes.get("product", {}).get(
                    k, ""
                )
            #
            for doc_id in self.getRelatedRfTree(attachment_id.id):
                sub_attrs = get_clone_info_attr(doc_id)
                out["RF"].append((main_parent_attrs, sub_attrs))
                for doc_id_2d in self.getRelatedLyTree(
                    doc_id, optional_return_type=["2d"]
                ):
                    doc_ids_2d.append((sub_attrs, doc_id_2d))
            #
            for doc_id_2d in self.getRelatedLyTree(
                attachment_id.id, optional_return_type=["2d"]
            ):
                doc_ids_2d.append((main_parent_attrs, doc_id_2d))
                #
            for parent_attrs, doc_id in doc_ids_2d:
                out["LF"].append((parent_attrs, get_clone_info_attr(doc_id)))
        #
    ...
    def getDocumentChechOutDict(self, document_id):
        out_template = {
            ...
            "file_name": "",
            "message": "template",
            "check_in_out_flag": {"in": False, "out": False, "out_user": {}},
        }
    ...
    @api.model
    def getCheckOutInState(self, args):
        """
    ...
    def getOutUserInfo(self):
        ...
        for checkOutBrws in checkoutBrwsList:
            return {
                "name": self.getUserSign(checkOutBrws.userid.id),
    ...
    def GetCloneDocumentValues(self, args):
        ...
        if hasattr(self, "customGetCloneDocumentValues"):
            ...
            #
            out_attachment_value = self.customGetCloneDocumentValues(
                out_attachment_value, json.loads(old_product_attrs), new_product_attrs
            )
        else:
            #
            ...
            if "INTEGRATION_FILE_EXTE" in out_attachment_value:
                ...
                _, exte = os.path.splitext(
                    out_attachment_value["INTEGRATION_ORIG_FILE_PATH"]
                )
        ...
        if "created" not in out_attachment_value:
            out_attachment_value["created"] = datetime.now().strftime(
                DEFAULT_SERVER_DATETIME_FORMAT
            )
    ...
    def check_attachment_change_impact(self):
        ...
        return self.env.ref("plm.action_report_attachment_change_impact").report_action(
            docids=[self.id], data=report_data
        )

    def getRelatedLyTreeNew(
        self, optional_return_type=["3d"], getOnyChkOut=False, latest=False
    ):
        """ """
        #
        self.ensure_one()
        ...
        for doc_rel_id in doc_rel_ids:
            if parent_doc_type == "3d":
                if (
                    doc_rel_id.parent_id.id == self.id
                    and doc_rel_id.child_id.document_type == "2d"
                ):
                    if latest:
                    ...
                    doc_rel_id.child_id.id == self.id
                    and doc_rel_id.parent_id.document_type == "2d"
                ):
                    if latest:
    ...
    @api.model
    def getRelatedHiTreeNew(
        self, recursion=True, getRftree=False, getOnyChkOut=False, latest=False
    ):
        """
        Get children HiTree documents
        """
        ...
        #
        def _getRelatedHiTree(attachment_id, recursion, getRftree):
            if latest:
                attachment_id = attachment_id.get_latest_version()[0]
            children_attachment_ids = self.env["ir.attachment.relation"].search(
                [("link_kind", "=", "HiTree"), ("parent_id", "=", attachment_id.id)]
            )
            for child_attachment_id in children_attachment_ids:
            ...
            if getRftree:
                for obj in self.getRelatedRfTreeNew(
                    recursion=True, evaluated=[], latest=latest
                ):
                    out["data"] += obj

        _getRelatedHiTree(self, recursion, getRftree)
        return out["data"]

    @api.model
    def getRelatedRfTreeNew(self, recursion=True, evaluated=False, latest=False):
        #
        self.ensure_one()
        if not evaluated:
        ...
        out = self.env["ir.attachment"]
        #
        doc_rel_ids = self.env["ir.attachment.relation"].search(
            [("link_kind", "in", ["RfTree"]), ("parent_id", "=", self.id)]
        )
        if self.id in evaluated.ids:
            logging.warning(
                "Document %r already found in RfTree evaluated %r" % (self, evaluated)
            )
            return out
        evaluated += self
        #
    ...
    def get_last_cad_save_date(self):
        ...
        """
        last_write_date = (
            self.sudo().env["plm.cad.open"].getLastCadSave(self).write_date
        )
    ...
    def get_last_my_open(self):
        last_open_date = (
            self.sudo()
            .env["plm.cad.open"]
            .getLastCadOpenByUser(self, self.env.user)
            .write_date
        )
    ...
    def isDownloadabeFromServer(self):
        self.ensure_one()
        is_last_version = not self.engineering_state not in [
            OBSOLATED_STATUS,
            UNDER_MODIFY_STATUS,
        ]
        if is_last_version:
    ...
    def is_open_by_me(self, hostname, pws_path):
        #
    ...
    def isCollectableNew(self, hostname, pws_path):
        self.ensure_one()
        is_latest_version = self.engineering_state not in [
            OBSOLATED_STATUS,
            UNDER_MODIFY_STATUS,
        ]
        if is_latest_version:
            if self.isCheckedOutByMe():
                return "CHECK-OUT"
            if self.is_open_by_me(hostname, pws_path):
    ...
    def get_download_dict(self, hostname, pws_path):
        """
        ...
        return {
            ...
            "last_update": self.get_last_cad_save_date().strftime(
                DEFAULT_SERVER_DATETIME_FORMAT
            ),
            "last_my_open": self.get_last_my_open().strftime(
                DEFAULT_SERVER_DATETIME_FORMAT
            ),
            "is_downloadable": self.isCollectableNew(hostname, pws_path),
            "is_last_revision": (
                True
                if self.engineering_state not in [OBSOLATED_STATUS, UNDER_MODIFY_STATUS]
                else False
            ),
        }

    def get_all_relation_flat_structure_sql(self, link_kind="HiTree"):
        """
        ...
        ids = set()
        ids.add(self.id)
        self.env.cr.execute(sql, (self.id, self.id, link_kind))
        for child_id in self.env.cr.fetchall():
            ids.add(child_id[0])
        return self.browse(list(ids))

    def download_structure(self, hostname, hostpws, latest=False):
        """
        ...
        if latest:
            for document_id in self.getDocBomFlat(latest):
                out.append(document_id.get_download_dict(hostname, hostpws))
        else:
