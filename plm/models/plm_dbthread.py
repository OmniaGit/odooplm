...
class PlmDbthread(models.Model):
    ...
    documement_name_version = fields.Char(
        string="Document name and version", readonly=True, index=True
    )
    threadCode = fields.Char(
        "Thread Code assigned automatically", readonly=True, index=True
    )
    done = fields.Boolean("This thread is done ?", readonly=True, index=True)
    ...
    def getNewThreadTransaction(self, list_doc):
        ...
        for docDict in list_doc[0]:
            ...
            if name:
                key = "%s_%s" % (
                    docDict.get("engineering_code"),
                    docDict.get("engineering_revision", 0),
                )
                if key:
                    self.create(
                        {"documement_name_version": key, "threadCode": threadCode}
                    )
        return threadCode

    @api.model
    def cleadUpPrevious(self, document_key, plm_dbthread_id):
        for plm_dbthread_id in self.search(
            [
                ("id", "<", plm_dbthread_id),
            ...
            plm_dbthread_id.error_message = (
                "Automatically close from tread %s " % self.threadCode
            )
    ...
    def notifieDoneToDbThread(self, clientArgs):
        ...
        for plm_dbthread_id in self.search(
            ...
            return True
        logging.warning(f"Try to update {clientArgs[0]} but not found in the db")
        return False
    ...
    def freezeDbThread(self, clientArgs):
        ...
        for plm_dbthread_id in self.search(
            [("threadCode", "=", dbThread), ("done", "=", False)]
        ):
            plm_dbthread_id.done = True
    ...
    def get_last_dbthread(self, document_key):
        for dbthread in self.search(
            [("documement_name_version", "=", document_key)], order="write_date desc"
        ):
            return dbthread
