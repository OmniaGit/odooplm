...
class PlmBackupDocument(models.Model):
    ...
    documentid = fields.Many2one("ir.attachment", "Related Document")
    ...
    def action_restore_document(self):
        ...
        for plm_backupdoc_id in plm_backupdoc.browse(plm_backupdoc_ids):
            ...
            if ir_attachment_bck_id:
                ...
                sql = "UPDATE IR_ATTACHMENT SET store_fname = %s where id = %s"
                self.env.cr.execute(
                    sql, (plm_backupdoc_id.existingfile, ir_attachment_bck_id.id)
                )
                if write_res:
                    logging.info(
    ...
    def action_restore_all_documents(self):
        ...
        if len(active_ids) != 1:
            raise ValidationError(
                _("Please select exactly one backup document to restore.")
            )
        ...
        tree_ids.update(
            ir_att.getRelatedHiTree(root_id, recursion=True, getRftree=True)
        )
        ...
        if not_checked_in_found:
            raise ValidationError(_("All documents must be checked-in before restore."))
        ...
        for att, bck in ready_pairs:
            ...
            if write_res:
                logging.info(
                    ...
                    att.id,
                    att.engineering_code,
                    att.engineering_revision,
                    bck.id,
                    bck.create_date,
                )
            else:
        ...
        if not restored_ids:
            raise ValidationError(
                _("No documents were restored. Please check the server logs.")
            )
