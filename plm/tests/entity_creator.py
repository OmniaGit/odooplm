...
class PlmEntityCreator:
    ...
    def create_product_template(self, name):
        ...
        default_data = ProductTemplate.default_get(
            ["uom_id", "uom_po_id", "sale_line_warn"]
        )
        default_data.update(
            {
                "name": name,
    ...
    def create_product_product(self, name, eng_code=False):
        ...
        for pp in self.env["product.product"].search(
    ...
    def create_bom_2_level(self, name):
        ...
        self.env["mrp.bom.line"].create(
            {"bom_id": parent_bom.id, "product_id": p_product1.id, "product_qty": 1}
        )
        ...
        self.env["mrp.bom.line"].create(
            {"bom_id": child_bom.id, "product_id": p_product2.id, "product_qty": 1}
        )
    ...
    def create_product_document(self, name, doc_type="3d"):
        ...
        document = self.create_document(
            f"attachment_{doc_type}_{name}", doc_type=doc_type
        )
    ...
    def create_bom(self, parent, child, qty=1):
        ...
        if not parent_bom:
            parent_bom = self.env["mrp.bom"].create(
                {"product_id": parent.id, "product_tmpl_id": parent.product_tmpl_id.id}
            )
        self.env["mrp.bom.line"].create(
            {"bom_id": parent_bom.id, "product_id": child.id, "product_qty": qty}
        )
        return parent_bom
    ...
    def create_bom_with_document(self, name):
        ...
        self.env["mrp.bom.line"].create(
            {"bom_id": parent_bom.id, "product_id": c1_product.id, "product_qty": 1}
        )
        child_bom = self.env["mrp.bom"].create(
            {
        ...
        self.env["mrp.bom.line"].create(
            {"bom_id": child_bom.id, "product_id": c2_product.id, "product_qty": 1}
        )
        return parent_bom
    ...
    def get_3_level_assembly(self, name=""):
        #
        product1, document3D1, document2D1, documentRD1 = (
            self.create_product_document_with_layout_rfTree(f"{name}_Parent")
        )
        product2, document3D2, document2D2, documentRD2 = (
            self.create_product_document_with_layout_rfTree(f"{name}_Child_level_1")
        )
        product3, document3D3, document2D3, documentRD3 = (
            self.create_product_document_with_layout_rfTree(f"{name}_Child_level_1.1")
        )
        product4, document3D4, document2D4, documentRD4 = (
            self.create_product_document_with_layout_rfTree(f"{name}_Child_level_2")
        )
        product5, document3D5, document2D5, documentRD5 = (
            self.create_product_document_with_layout_rfTree(f"{name}_Child_level_2.1")
        )
        #
        # create doc bom
        #
