...
class PlmMultiCompanyAccess(TransactionCase):
    ...
    def test_a_redefined_node_opens_to_its_own_groups(self):
        ...
        self.user_a.groups_id = [Command.link(self.group_purchase.id)]
        self.assertTrue(self._can_read(self.user_a, document))
    ...
    def test_cad_client_create_uses_the_node(self):
        self.user_a_rd.plm_access_id = self.rd_prototypes
        # Writing res.users drops Odoo's group definitions, and rebuilding them
        # from inside the create below raises a KeyError on a database where a
        # PLM group implies an mrp one. Rebuilt here instead, it succeeds; the
        # create then finds them cached. Odoo's own SetDefinitions is what
        # breaks, not the code under test, which the same steps in a shell run
    ...
    def test_upgrade_attaches_documents_left_with_res_id_zero(self):
        ...
        the script of 18.0.8.0.1 looked for NULL alone. Those documents kept no
        node, and since read access is granted through the node they hang from,
        no company rule ever reached them."""
        ...
        path = file_path("plm/upgrades/18.0.21.0.29/post-migrate.py")
        spec = importlib.util.spec_from_file_location("plm_post_migrate_29", path)
        script = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(script)
        script.migrate(self.env.cr, "18.0.21.0.28")
        self.env.invalidate_all()
