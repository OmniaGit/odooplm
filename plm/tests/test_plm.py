...
class PlmDateBom(TransactionCase, PlmEntityCreator):
    ...
    def test_1_some_wk(cls):
        ...
        # create main product
        #
        cls.product_parent_id = cls.create_product_product(
            product_product_name, product_product_code
        )
        #
        # release the product
        #
        ...
        assert (
            cls.product_parent_id.product_tmpl_id.get_latest_version().engineering_revision
            == 0
        )
        assert (
            cls.product_parent_id.product_tmpl_id.get_previus_version().engineering_revision
            == 0
        )
        assert (
            cls.product_parent_id.product_tmpl_id.get_next_version().engineering_revision
            == 0
        )
        assert (
            cls.product_parent_id.product_tmpl_id.get_released().engineering_revision
            == 0
        )
        assert (
            cls.product_parent_id.product_tmpl_id.get_released().engineering_code
        ...
        # work on copy
        #
        product_product_copy = cls.product_parent_id.product_tmpl_id.copy(
            {"name": "product_product_copy"}
        )
        ...
        #
        assert (
            cls.product_parent_id.product_tmpl_id.get_previus_version()
        ...
        cls.product_tmpl_copy = cls.product_tmpl_1.copy(
            {"name": "test_product_template_copy"}
        )
    ...
    def test_3_product_attachment_wk(self):
        ...
        assert document.engineering_state == START_STATUS, (
            "status is %s" % document.engineering_state
        )
        #
        # test release
        #
        ...
        assert product.engineering_state == RELEASED_STATUS, (
            "status is %s" % product.engineering_state
        )
        assert document.engineering_state == RELEASED_STATUS, (
            "status is %s" % document.engineering_state
        )
    ...
    def test_4_bom_wk(self):
        ...
        children_ids = self.env["product.product"]._getChildrenBom(
            root_product_id, level=1
        )
