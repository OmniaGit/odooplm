...
from odoo.addons.plm.tests.entity_creator import DUMMY_CONTENT, PlmEntityCreator
...
class PreCheckInCorrectnessTest(TransactionCase, PlmEntityCreator):
    ...
    def test_all_checked_out_by_me(self):
        ...
        self.assertEqual(
            len(res["info"]), 0, f"Expected 0 info entries, got {len(res['info'])}"
        )
...
class PreCheckInTimingTest(TransactionCase):
    ...
    def setUp(self):
        ...
        _batch_checkout(self.env, self.all_docs, self.env.uid)
        _logger.warning(
            "[perf_test] checkout batch=%.3fs  docs=%d",
