...
class component_spare_parts_report(report_int):
    ...
    def getBomRows(self,cr, uid,parentID, context=None):
        """
           Return the first level bom fields
        """
        component=self.pool.get('product.product').browse(cr, uid, parentID, context=context)
        relType=self.pool.get('mrp.bom')
        for bomid in component.bom_ids:
            return relType.browse(cr, uid, bomid.id, context=context).bom_lines 
        return []
    ...
    def getSparePartsPdfFile(self,cr,uid,context,component,output,componentTemplate):
        ...
        if len(children)>0:
            pageStream=self.createBom(self.getBomRows(cr,uid,component.id),component.name,component.description)
            output.addPage(pageStream)
