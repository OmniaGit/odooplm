...
export class PlmMrpWorksheet extends MrpWorksheet {
    async setup() {
        ...
        if (
            ...
            this.props.record.data.operation_id &&
            this.props.record.data.operation_id.length != 0
        ) {
            const displayDocs = await this.props.record.model.orm.read(
    ...
    async clicked() {
        const action_open_workorder_kanban = await this.props.record.model.orm.call(
            "mrp.workorder",
            "action_open_workorder_kanban",
